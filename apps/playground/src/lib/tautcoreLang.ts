...
interface TautcoreState {
  inString: boolean;
}

const parser: StreamParser<TautcoreState> = {
...
export const tautcoreStreamLanguage = StreamLanguage.define(parser);
...
export function tautcoreCompletions(context: CompletionContext): CompletionResult | null {
...
function unifiedCompletions(context: CompletionContext): CompletionResult | null {
  return contextAwareCompletions(context) ?? tautcoreCompletions(context);
}

export function tautcore(): LanguageSupport {
  return new LanguageSupport(tautcoreStreamLanguage, [
    diagramContextField,
    tautcoreStreamLanguage.data.of({ autocomplete: unifiedCompletions }),
  ]);
}
