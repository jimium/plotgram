...
export function getPreviewHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
  const wasmJsUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, 'media', 'wasm', 'tautcore_wasm.js'),
  );
  const wasmBinUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, 'media', 'wasm', 'tautcore_wasm_bg.wasm'),
  );
...
<head>
  ...
  <meta id="tautcore-config" data-wasm-js="${escapeHtmlAttr(wasmJsUri.toString())}" data-wasm-bin="${escapeHtmlAttr(wasmBinUri.toString())}" />
  <title>Tautcore Preview</title>
  <style>
    body {
      margin: 0;
      padding: 16px;
