# VS Code 插件需求：Tautcore DSL 集成

> 版本：0.1.0-draft | 状态：规划中

## 目标

在 VS Code 中为 Tautcore DSL（`.dfy`）提供**语言级编辑体验**和**图形化预览能力**，并支持在 Markdown 文档中**内嵌并渲染** DSL 图表，让「写代码 → 看图形 → 写文档」在同一工作流里完成。
...
- Language Server（`crates/tautcore-lsp`，待建）负责诊断，复用 `tautcore-core` 的 Parser / Validator
...
- 渲染链路复用 `tautcore-wasm`，产物放入 `media/`（构建时从 `crates/tautcore-wasm` 复制）
...
**目标：** 在 Markdown 文档里写 Tautcore 代码块，预览时显示为图形而非纯文本。

| 能力 | 说明 |
|------|------|
| 代码块识别 | 支持 ` ```tautcore ` 或 ` ```pgm ` 等 fenced code block |
...
- 注册 `markdown.markdownItPlugins`，在扩展宿主侧用 Node WASM 渲染 `tautcore` 代码块并注入 SVG（避免 Markdown Preview Webview 的 CSP 限制）
...
flowml/
├── crates/
│   ├── tautcore-core/     # Parser、Validator、Renderer
│   ├── tautcore-wasm/     # WASM 绑定（预览 & Markdown 渲染）
│   └── tautcore-lsp/      # 待建：LSP 语言服务
├── editors/
│   └── vscode/           # 本扩展
...
| **V1** | LSP 语义校验 + 预览实时同步 + Markdown `tautcore` 代码块渲染 |
...
|----|------|
| 文件扩展名 | 仅 `.dfy` |
| Markdown 代码块语言标识 | `tautcore` |
| 预览渲染方式 | 本地 WASM（`tautcore-wasm`） |
| 离线能力 | 是，WASM 打包进扩展，不依赖网络 |
| WASM 产物 | 在 `editors/vscode` 内独立构建，不复用 `playground/tautcore-wasm` |
...
| Markdown `tautcore` 代码块渲染 | ✅ |
| LSP（`tautcore-lsp`） | 待建（V1） |
| Quick Fix / 格式化 | 待建（V2+） |
