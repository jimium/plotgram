 ...
 *
 * 注意:diff_sources / apply_patch / ast_to_source 需要在 tautcore-wasm crate
...
export function loadWasm(): Promise<TautcoreWasm> {
  if (!modulePromise) {
    modulePromise = (async () => {
      // WASM 产物由仓库根目录的 wasm-pack 生成到 studio/tautcore-wasm/
 ...
 * 比较两份 DSL 源码的差异
 *
 * 依赖 tautcore-wasm 新增的 diff_sources 绑定
 */
 ...
 * 对 DSL 应用增量补丁
 *
 * 依赖 tautcore-wasm 新增的 apply_patch 绑定
 */
export function applyPatch(
  ...
  if (typeof wasm.apply_patch !== 'function') {
    return {
      ...
      errors: ['当前 WASM 版本不支持 apply_patch,请重新构建 tautcore-wasm'],
    };
  }
...
export function checkStudioCapabilities(wasm: TautcoreWasm): {
  diff: boolean;
  applyPatch: boolean;
  astToSource: boolean;
} {
