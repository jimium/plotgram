...
Tautcore Studio 是纯前端应用,构建产物为静态文件,可部署到任意静态文件服务器。

### 1.1 前置条件

- Node.js 18+
- 已构建 tautcore-wasm 产物(见下文)
...
```bash
cd /path/to/flowml/crates/tautcore-wasm
wasm-pack build --target web --release --out-dir ../../studio/tautcore-wasm
```

构建后 `studio/tautcore-wasm/` 目录包含:
- `tautcore_wasm.js`(JS 胶水代码)
- `tautcore_wasm_bg.wasm`(WASM 二进制)
- `tautcore_wasm.d.ts`(类型声明)
...
- `index.html`
- `assets/`(JS、CSS、图片)
- `tautcore-wasm/`(WASM 产物,需手动复制或配置构建工具)
...
server {
    listen 80;
    server_name studio.tautcore.example.com;
    root /var/www/tautcore-studio;
    index index.html;
...
```bash
docker build -t tautcore-studio .
docker run -p 8080:80 tautcore-studio
```
...
WASM 产物位于 `studio/tautcore-wasm/`,由 wasm-pack 生成,不入版本控制。
...
WASM 产物版本由 tautcore-wasm 的 Cargo.toml 决定。Studio 通过 `wasm.version()` 获取版本号并显示在顶栏。
...
A: 检查 `studio/tautcore-wasm/` 目录是否存在产物,以及服务器是否正确配置 `.wasm` 的 MIME 类型为 `application/wasm`。
...
A: 当前 WASM 产物未包含 `apply_patch` 绑定。需在 tautcore-wasm crate 新增导出后重新构建(见 [tasks.md](tasks.md) P1 阶段)。
