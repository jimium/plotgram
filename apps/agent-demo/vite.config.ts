...
// Tautcore Agent Demo 开发服务器配置
// WASM 产物由仓库根目录的 tautcore-wasm crate 提供，通过相对路径引用
// 开发时 /agent/chat 代理到本地 tautcore-server (6080)
// 生产构建走 CDN：assets/* 由 VITE_CDN_BASE 重写，wasm 走 tautcore-wasm/ 子目录
const rootDir = fileURLToPath(new URL('.', import.meta.url));
const wasmDir = resolve(rootDir, 'tautcore-wasm');
const cdnBase = process.env.VITE_CDN_BASE || '';

/**
 * 开发/预览时从 `agent-demo/tautcore-wasm/` 提供 `/tautcore-wasm/*`。
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '../tautcore-wasm/tautcore_wasm.js': resolve(wasmDir, 'tautcore_wasm.js'),
    },
  },
  server: {
    port: 3200,
    strictPort: false,
    // 代理 Agent 中转请求到 tautcore-server，避免跨域
    proxy: {
      '/agent': {
