#!/usr/bin/env bash
# Slice 3/4 行为不变验证：把 showcase 全量 .pgm 渲染为 SVG 到指定目录（扁平命名）。
# 用法：route_snapshot.sh <out_dir>
# 需先 `cargo build -p plotgram-cli`（debug）。不改动 showcase 目录。
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BIN="$ROOT_DIR/target/debug/plotgram"
...
while IFS= read -r -d '' f; do
  ...
  count=$((count + 1))
done < <(find "$ROOT_DIR/showcase" -name '*.pgm' -not -path '*/.*' -print0 | sort -z)
