...
//! cross-axis coordinates (median relaxation + VPSC) → orthogonal Ink.
//!
...
impl LayoutAlgorithm for HierarchicalLayout {
    ...
    fn layout(&self, input: LayoutInput<'_>) -> Result<LayoutOutput, LayoutError> {
        ...
        let ports = compose::ports::assign_ports(&real_graph, &plan);
...
fn canonical_rect_to_physical(o: AlgoOrientation, r: Rect) -> Rect {
    let corners = [
        Point { x: r.x, y: r.y },
        Point {
            x: r.right(),
...
/// coordinates — see [`canonical_rect_to_physical`]).
