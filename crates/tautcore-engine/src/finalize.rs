...
use tautcore_layout::layout::hierarchical::{GROUP_LABEL_TOP_PAD, GROUP_PAD};
use tautcore_model::diagnostics::LayoutDiagnostics;
...
use tautcore_router::core::union_rects;
...
fn collect_group(
    group: &tautcore_model::graph::Group,
    nodes: &[NodePlacement],
...
fn group_member_node_ids(group: &tautcore_model::graph::Group) -> Vec<String> {
    let mut ids = Vec::new();
...
fn find_group<'a>(graph: &'a Graph, id: &str) -> Option<&'a tautcore_model::graph::Group> {
    fn walk<'a>(
        g: &'a tautcore_model::graph::Group,
        id: &str,
    ) -> Option<&'a tautcore_model::graph::Group> {
        if g.id == id {
            return Some(g);
        }
        for c in &g.groups {
...
mod tests {
    use super::*;
    use tautcore_model::graph::{Group, Node};
    ...
    fn provided_groups_are_not_recomputed() {
        let node = Node {
            ...
            label: None,
            shape: None,
            role: tautcore_model::graph::NodeRole::Entity,
            host_group: None,
            anchor: None,
            partition_cell: None,
            attrs: tautcore_model::attr::AttrMap::new(),
        };
        let graph = Graph {
