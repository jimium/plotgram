...
/// 线段是否与矩形相交
///
/// 委托到 `plotgram_core::layout::refine::segment_intersects_node`，
...
fn compute_group_gap_deficit(diagram: &Diagram, result: &LayoutResult) -> f64 {
    ...
    let groups: Vec<(&String, &plotgram_core::layout::GroupLayout)> = result.groups.iter().collect();
...
mod tests {
    use super::*;
    use plotgram_core::layout::{PathGeometry, Port};
