...
//!
//! 用法:
//!   cargo run -p plotgram-eval --bin v2-effectiveness -- --showcase /path/to/showcase --stress /path/to/friendliness_stress
...
fn load_dir(dir: &Path, into: &mut Vec<(String, plotgram_core::ast::Diagram)>) {
...
fn main() {
    ...
    for (name, diagram) in &diagrams {
        ...
        for (i, config) in layout_configs.iter().enumerate() {
            ...
            // V2 关闭
            std::env::set_var("PLOTGRAM_NO_V2_ADJUST", "1");
            ...
            // V2 开启
            std::env::remove_var("PLOTGRAM_NO_V2_ADJUST");
