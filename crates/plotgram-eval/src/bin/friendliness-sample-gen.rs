//! Phase 1.5 压力样本生成器
//!
//! 生成多样化的合成 .pgm 图文件到指定目录（默认 benchmarks/friendliness_stress），
...
//!
//! 用法:
//!   cargo run -p plotgram-eval --bin friendliness-sample-gen -- /path/to/friendliness_stress
...
fn main() {
    ...
    for &n in &[6usize, 8, 10, 12, 16, 20, 24, 30] {
        buf.clear();
        gen_chain(&mut buf, n);
        write_file(out_path, &format!("chain-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &(r, c) in &[(3usize, 3usize), (3, 4), (4, 4), (4, 5), (5, 5), (5, 6), (6, 6)] {
        ...
        write_file(out_path, &format!("grid-{}x{}.pgm", r, c), &buf);
        count += 1;
    }
    ...
    for &n in &[6usize, 8, 10, 12, 16, 20, 24] {
        buf.clear();
        gen_star(&mut buf, n);
        write_file(out_path, &format!("star-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &(l, r) in &[(3usize, 3usize), (3, 4), (4, 4), (4, 5), (5, 5), (5, 6), (6, 6)] {
        ...
        write_file(out_path, &format!("bipartite-{}x{}.pgm", l, r), &buf);
        count += 1;
    }
    ...
    for &(depth, branch) in &[(2usize, 3usize), (3, 2), (3, 3), (3, 4), (4, 2), (4, 3)] {
        ...
        if let Some(s) = gen_tree(&mut buf, depth, branch) {
            write_file(out_path, &format!("tree-d{}b{}.pgm", depth, branch), &buf);
            count += 1;
            let _ = s;
        }
    }
    ...
    for &layers in &[3usize, 4, 5, 6] {
        for &per_layer in &[3usize, 4, 5] {
            ...
            write_file(out_path, &format!("dag-L{}P{}.pgm", layers, per_layer), &buf);
            count += 1;
        }
    }
    ...
    for &ng in &[3usize, 4, 5, 6] {
        ...
        write_file(out_path, &format!("multigroup-g{}.pgm", ng), &buf);
        count += 1;
    }
    ...
    for &layers in &[2usize, 3, 4] {
        for &spokes in &[3usize, 4, 5] {
            ...
            write_file(out_path, &format!("hublayer-L{}S{}.pgm", layers, spokes), &buf);
            count += 1;
        }
    }
    ...
    for &n in &[10usize, 14, 18, 22, 26] {
        ...
        write_file(out_path, &format!("dense-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &(layers, per_layer) in &[(2usize, 4usize), (2, 5), (2, 6), (3, 4), (3, 5), (4, 3)] {
        ...
        write_file(out_path, &format!("wide-L{}P{}.pgm", layers, per_layer), &buf);
        count += 1;
    }
    ...
    for &n in &[5usize, 6, 7, 8, 10, 12] {
        ...
        write_file(out_path, &format!("state-cycle-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &n in &[3usize, 4, 5, 6, 8] {
        ...
        write_file(out_path, &format!("er-schema-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &n in &[5usize, 6, 7, 8, 10, 12, 14] {
        buf.clear();
        gen_ring(&mut buf, n);
        write_file(out_path, &format!("ring-n{}.pgm", n), &buf);
        count += 1;
    }
    ...
    for &n in &[8usize, 10, 12, 14, 16, 20] {
        ...
        write_file(out_path, &format!("pathsc-n{}.pgm", n), &buf);
        count += 1;
    }
    // 更多 chain 变体
    for &n in &[4usize, 5, 7, 9, 14, 18, 22, 28] {
        buf.clear();
        gen_chain(&mut buf, n);
        write_file(out_path, &format!("chain2-n{}.pgm", n), &buf);
        count += 1;
    }
    // 更多 star 变体
    for &n in &[4usize, 5, 7, 9, 14, 18, 22] {
        buf.clear();
        gen_star(&mut buf, n);
        write_file(out_path, &format!("star2-n{}.pgm", n), &buf);
        count += 1;
    }
    // 更多 bipartite 变体
    for &(l, r) in &[(2usize, 3usize), (2, 4), (3, 3), (4, 3), (3, 5), (4, 4), (5, 4)] {
        buf.clear();
        gen_bipartite(&mut buf, l, r);
        write_file(out_path, &format!("bipartite2-{}x{}.pgm", l, r), &buf);
        count += 1;
    }
    // 更多 grid 变体
    for &(r, c) in &[(2usize, 3usize), (2, 4), (3, 3), (3, 5), (4, 3), (4, 4), (5, 4)] {
        buf.clear();
        gen_grid(&mut buf, r, c);
        write_file(out_path, &format!("grid2-{}x{}.pgm", r, c), &buf);
        count += 1;
    }
    // 更多 DAG 变体（不同 seed）
    for &layers in &[3usize, 4, 5] {
        for &per_layer in &[2usize, 3, 4] {
            ...
            write_file(out_path, &format!("dag2-L{}P{}.pgm", layers, per_layer), &buf);
            count += 1;
        }
    }
    // 更多 dense 变体
    for &n in &[6usize, 8, 10, 12, 16, 20] {
        buf.clear();
        gen_dense_flow(&mut buf, n);
        write_file(out_path, &format!("dense2-n{}.pgm", n), &buf);
        count += 1;
    }
    // 更多 state-cycle
    for &n in &[4usize, 6, 9, 11, 14] {
        buf.clear();
        gen_state_cycle(&mut buf, n);
        write_file(out_path, &format!("state-cycle2-n{}.pgm", n), &buf);
        count += 1;
    }
    // 更多 er-schema
    for &n in &[2usize, 4, 6, 7, 9, 10] {
        buf.clear();
        gen_er_schema(&mut buf, n);
        write_file(out_path, &format!("er-schema2-n{}.pgm", n), &buf);
        count += 1;
    }
    // 双 hub 星形（两个中心节点）
    for &n in &[6usize, 8, 10, 12, 16] {
        ...
        write_file(out_path, &format!("dualhub-n{}.pgm", n), &buf);
        count += 1;
    }
    // 层间稀疏连接
    for &(layers, per_layer) in &[(3usize, 3usize), (4, 2), (3, 4), (5, 2), (4, 3)] {
        ...
        write_file(out_path, &format!("sparse-L{}P{}.pgm", layers, per_layer), &buf);
        count += 1;
    }

    eprintln!("✓ 生成 {} 个 .pgm 文件到 {}", count, out_dir);
}
