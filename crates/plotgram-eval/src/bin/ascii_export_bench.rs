use plotgram_core::ast::{PreparedDiagram, RawDiagram};
use plotgram_core::prepare::StyleRequest;
use plotgram_core::render::encode::ascii::{
...
"#;
    ...
    for _ in 0..iterations {
        ...
        let start = Instant::now();
        let result = plotgram_core::render::encode::ascii::encode_with_report(&request)
...
fn prepare_diagram(source: &str) -> PreparedDiagram {
    let raw = plotgram_core::pipeline::parse(source).expect("parse diagram source");
