//! Plotgram Server HTTP API handlers
...
/// 解析 + prepare + 校验 Plotgram 源码，返回 PreparedDiagram 与结构化诊断结果。
fn check_source(source: &str) -> (Option<PreparedDiagram>, CheckResult) {
    let output = plotgram_core::pipeline::parse_prepare_validate(source, &StyleRequest::default());
