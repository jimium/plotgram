...
//! .taut source
//!   → tautcore-parse
//!   → measure NodeSizes (+ ContentLayout) via tautcore-content / label heuristics
//!   → tautcore_engine::run
//!   → tautcore-render (SVG)
//! ```
//!
...
pub use tautcore_layout::LayoutDebugTrace;

use tautcore_engine::run as run_layout;
use tautcore_engine_api::{EdgeGeometryMode, LayoutInput};
use tautcore_model::render::RenderInput;
use tautcore_model::result::LayoutResult;
use tautcore_parse::parse;
use tautcore_render::{render_drawio, render_svg};

use crate::measure_graph::measure_node_sizes;

/// Build `.taut` source into an SVG string.
pub fn build_svg(source: &str, options: &BuildOptions) -> Result<String, BuildError> {
    Ok(build_svg_with_layout(source, options)?.1)
}

/// Build `.taut` source into a draw.io (mxGraphModel) XML string.
...
/// Build source into [`tautcore_model::result::LayoutResult`] only (no SVG).
pub fn build_layout(
    source: &str,
    options: &BuildOptions,
) -> Result<tautcore_model::result::LayoutResult, BuildError> {
    let parsed = parse(source)?;
    let node_sizes = measure_node_sizes(&parsed.graph, options)?;
    let contract = parsed.into_contract(node_sizes);
...
/// Build source into the hierarchical [`tautcore_layout::LayoutDebugTrace`]
...
pub fn build_debug_trace(
    ...
    match name.as_str() {
        "hierarchical" => {
            ...
            Ok(tautcore_layout::build_debug_trace(input, &name)?)
        }
