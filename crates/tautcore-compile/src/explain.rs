...
use tautcore_model::graph::{Graph, NodeRole};
...
pub fn explain(graph: &Graph, layout: &LayoutResult) -> String {
    ...
    // ── bands (geometric layering; the one fact .taut cannot tell) ──
...
mod tests {
    ...
    /// Observable-output tests: run the full pipeline on inline `.taut` and
