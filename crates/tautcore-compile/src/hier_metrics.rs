...
use tautcore_model::diagnostics::HierarchicalObs;
use tautcore_model::geometry::Point;
use tautcore_model::result::LayoutResult;
use tautcore_router::core::overlap_len;
...
impl CrossAxis {
    ...
    /// Best-effort parse from `.taut` source (`direction: …` inside layout block).
    ...
    fn center(self, frame: &tautcore_model::geometry::Rect) -> f64 {
        match self {
    ...
    fn extent_lo_hi(self, frame: &tautcore_model::geometry::Rect) -> (f64, f64) {
        match self {
...
fn fan_metrics(
    by_id: &BTreeMap<&str, &tautcore_model::result::NodePlacement>,
