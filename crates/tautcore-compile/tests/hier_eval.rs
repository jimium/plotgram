...
use tautcore_compile::{
    build_debug_trace, build_layout, compute_hier_metrics, node_gap_from_source, BuildOptions,
    CrossAxis,
};
use tautcore_layout::layout::hierarchical::GROUP_PAD;
use tautcore_layout::{group_penetration_violations, GROUP_FRAME_GAP, PARTITION_EMPTY_BAND_MIN};
use tautcore_model::geometry::{Point, Rect};
use tautcore_model::graph::{Edge as ModelEdge, Group as ModelGroup};
...
fn collect_taut(dir: &Path, out: &mut Vec<PathBuf>) {
    let Ok(rd) = fs::read_dir(dir) else { return };
    for entry in rd.filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.is_dir() {
            collect_taut(&path, out);
        } else if path.extension().and_then(|e| e.to_str()) == Some("taut") {
            out.push(path);
        }
    }
}
...
fn hierarchical_showcase_geometry_invariants() {
    ...
    collect_taut(&dir, &mut entries);
    entries.sort();
    assert!(
        !entries.is_empty(),
        "expected showcase/hierarchical/**/*.taut fixtures"
    );
...
fn check_group_gates(name: &str, source: &str, result: &LayoutResult, failures: &mut Vec<String>) {
    ...
    let Ok(parsed) = tautcore_parse::parse(source) else {
        return;
    };
...
fn symmetry_axis_d2_representative_spines_collinear() {
    ...
    const CASES: &[(&str, &[&str])] = &[
        ("flat/product.flat-rest-api.taut", &["web", "lb", "api"]),
        (
            "flat/mech.constrain-flat-chain.taut",
            &["gw", "api", "worker"],
        ),
        (
            "flat/product.order-approval.taut",
            &["submit", "review", "check"],
        ),
    ];
    let dir = showcase_dir();
    ...
    for &(rel, spine) in CASES {
        ...
        let by_id: BTreeMap<&str, &tautcore_model::result::NodePlacement> =
...
fn symmetry_axis_d3_fan_pack_multi_rank_backedge() {
    let path = showcase_dir().join("flat/smoke.multi-rank-backedge.taut");
...
fn twin_spine_constrain_sink_side_on_axis() {
    let path = showcase_dir().join("flat/mech.constrain-sink.taut");
...
fn port_lane_three_tier_has_no_mid_gap_horizontal_jogs() {
    let path = showcase_dir().join("flat/product.three-tier.taut");
...
fn diamond_fanout_gateway_sources_stay_on_south() {
    let path = showcase_dir().join("flat/smoke.flat-gateway-fanout.taut");
    ...
    for e in &result.edges {
        ...
        if fp.side != tautcore_model::port::Side::South {
            failures.push(format!(
...
fn fan_out_four_cross_rails_nest_outer_inner() {
    let path = showcase_dir().join("flat/smoke.fan-out-four.taut");
...
fn ticket_triage_escalate_handle_side_corridor_west() {
    let path = showcase_dir().join("flat/product.ticket-triage.taut");
...
fn order_approval_primary_arm_on_spine_approved_left() {
    let path = showcase_dir().join("flat/product.order-approval.taut");
...
fn d22b_fallback_observability_and_demand_floor() {
    ...
    let cases: &[(&str, bool)] = &[
        ...
        ("group-weak/demo.tautcore-core-mod-deps", false),
    ...
    for &(rel, expect_fb) in cases {
        let path = showcase_dir().join(format!("{rel}.taut"));
