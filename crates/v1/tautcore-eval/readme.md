# tautcore-eval — Tautcore 布局与边路由算法评估框架
...
```rust
use tautcore_eval::engine::{EvalEngine, presets};
...
```rust
use tautcore_eval::history::HistoryStore;
...
# 布局+路由组合评估
tautcore-eval eval flowchart.taut -c combinations

# 批量评估目录
tautcore-eval batch showcase/

# 按图类型权重评估
tautcore-eval batch showcase/ -w per-type

# 与基线对比
tautcore-eval batch showcase/ --baseline .eval-history/baseline.json

# 保存历史
tautcore-eval batch showcase/ --save-history
