...
//!    结论），量化固定容量下的争用损失。
//!
//! 口径见 [`tautcore_core::layout::atlas::probe`]：统一 flat 网格，不镜像
//! divide-conquer / two_phase（有组的 flowchart/architecture 在报告中标注）。
//!
//! 用法：
//!   cargo run -p tautcore-eval --bin atlas_probe
//!   cargo run -p tautcore-eval --bin atlas_probe -- --set benchmarks/sets/product-regression-set.txt
//!   cargo run -p tautcore-eval --bin atlas_probe -- --output /tmp/atlas_probe.md

use tautcore_core::ast::Diagram;
use tautcore_core::layout::atlas::channel::{
...
use tautcore_core::layout::atlas::probe::derive_channel_blueprint;
use tautcore_core::layout::kernel::cost::SolverStatus;
...
/// 仓库根：`CARGO_MANIFEST_DIR`（=crates/tautcore-eval）的上两级。
fn repo_root() -> PathBuf {
    let manifest = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
    Path::new(&manifest).join("../..")
}

/// 读取图集清单（相对仓库根的 .taut 路径，跳过 `#` 注释与空行）。
...
fn render_gate_scan(diagrams: &[(String, Diagram)]) -> String {
    ...
    out.push_str(&format!(
        ...
        tautcore_core::layout::atlas::probe::DEFAULT_GATE_CAPACITY
    ));
