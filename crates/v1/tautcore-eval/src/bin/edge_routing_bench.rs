...
//!
//! 用法:
//!   cargo run -p tautcore-eval --bin edge_routing_bench
//!   cargo run -p tautcore-eval --bin edge_routing_bench -- --output edge_bench.json
//!   cargo run -p tautcore-eval --bin edge_routing_bench -- --showcase /path/to/showcase

use tautcore_core::ast::{DiagramAttribute, Diagram, AttributeValue, TextValue};
use tautcore_core::layout;
use tautcore_core::layout::routing::common::label_avoidance::{aabb_overlap, label_bbox};
use tautcore_eval::metrics::LayoutMetrics;
use serde::Serialize;
use std::fs;
use std::path::Path;
