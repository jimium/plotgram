use tautcore_core::ast::{PreparedDiagram, RawDiagram};
use tautcore_core::prepare::StyleRequest;
use tautcore_core::render::encode::ascii::{
    export_reader_to_writer, export_string, AsciiExportMetadata, AsciiExportOptions,
    AsciiNonAsciiPolicy,
};
use tautcore_core::render::{RenderFormat, RenderRequest};
use serde::Serialize;
use std::fs;
...
"#;
    ...
    for _ in 0..iterations {
        ...
        let start = Instant::now();
        let result = tautcore_core::render::encode::ascii::encode_with_report(&request)
...
fn prepare_diagram(source: &str) -> PreparedDiagram {
    let raw = tautcore_core::pipeline::parse(source).expect("parse diagram source");
    tautcore_core::pipeline::prepare(RawDiagram(raw.into_inner()), &StyleRequest::default())
