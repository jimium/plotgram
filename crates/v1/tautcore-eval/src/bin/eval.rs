//! tautcore-eval CLI 入口
//!
//! 用法:
//!   # 评估单个文件（按图表类型自动选择算法）
//!   tautcore-eval eval flowchart.taut
//!
//!   # 批量评估目录
//!   tautcore-eval batch showcase/
//!
//!   # 评估指定算法
//!   tautcore-eval eval flowchart.taut -a sugiyama
//!
//!   # 布局+路由组合评估
//!   tautcore-eval eval flowchart.taut --combinations
//!
//!   # 与历史基线对比
//!   tautcore-eval batch showcase/ --baseline .eval-history/baseline.json

use std::collections::HashMap;
use std::fs;
use std::path::{Path, PathBuf};

use tautcore_core::types::DiagramType;
use tautcore_eval::engine::presets;
use tautcore_eval::engine::EvalEngine;
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "-h" | "--help" => print_usage(),
        "-V" | "--version" => {
            println!("tautcore-eval {}", env!("CARGO_PKG_VERSION"));
        }
        _ => {
...
fn print_usage() {
    eprintln!(
        r#"tautcore-eval — Tautcore 布局与边路由算法评估框架

用法:
  tautcore-eval eval <file.taut> [选项]         评估单个文件
  tautcore-eval batch <目录> [选项]            批量评估目录下所有 .taut 文件
  tautcore-eval algo <算法名> <目录> [选项]    算法维度评估
  tautcore-eval diff <baseline.json> <current.json>  对比两次评估结果
  tautcore-eval baseline <目录> [-o target/showcase-baseline.json]  生成目录质量基线（可选工具；日常门禁见 benchmarks/）
  tautcore-eval baseline-check <目录> [--baseline <文件>]  与基线对比检测回归
...
算法维度评估 (algo):
  ...
  例: tautcore-eval algo sugiyama showcase/

示例:
  tautcore-eval eval showcase/flowchart/smoke.decision-loop.taut
  tautcore-eval eval showcase/flowchart/smoke.decision-loop.taut -a sugiyama
  tautcore-eval eval showcase/flowchart/smoke.decision-loop.taut -c combinations
  tautcore-eval batch showcase/ -o report.md
  tautcore-eval batch showcase/ -f json -o report.json
  tautcore-eval batch showcase/ --baseline baseline.json
  tautcore-eval algo sugiyama showcase/
  tautcore-eval diff baseline.json current.json
"#
    );
}
...
fn cmd_eval(args: &[String]) {
    ...
    let mut report = tautcore_eval::EvalReport::new(&format!("评估报告: {}", name));
...
fn cmd_batch(args: &[String]) {
    ...
    let mut diagrams: Vec<(String, tautcore_core::ast::Diagram, String)> = Vec::new();
    ...
    // 保存历史
    if cli.save_history {
        if let Ok(report) = serde_json::from_str::<tautcore_eval::EvalReport>(&content) {
...
/// 运行批量评估（按图表类型分组）
fn run_batch_eval(
    diagrams: &[(String, tautcore_core::ast::Diagram, String)],
    mode: &str,
    fmt: &str,
    weights_mode: &str,
) -> String {
    match mode {
        "auto" => {
            let mut report = tautcore_eval::EvalReport::new("Tautcore 布局与路由评估报告");

            // 按图表类型分组
            let mut by_type: HashMap<DiagramType, Vec<(String, tautcore_core::ast::Diagram, String)>> =
                HashMap::new();
        ...
        "combinations" => {
            let mut report = tautcore_eval::EvalReport::new("Tautcore 布局+路由组合评估报告");
        ...
        _ => {
            ...
            let mut report = tautcore_eval::EvalReport::new("Tautcore 评估报告");
            for (i, (name, diagram, source)) in diagrams.iter().enumerate() {
                eprintln!("[{}/{}] {}", i + 1, diagrams.len(), name);
...
fn cmd_algo(args: &[String]) {
    if args.len() < 2 {
        eprintln!("用法: tautcore-eval algo <算法名> <目录> [选项]");
        std::process::exit(1);
    }
    ...
    // 查找算法适用的图类型
    let applicable_types = tautcore_core::layout::diagram_types_for_layout(algo_name);
    ...
    let report = tautcore_eval::EvalReport::new(&format!("算法评估: {}", algo_name));
    let mut all_results: Vec<tautcore_eval::EvalResult> = Vec::new();
...
fn cmd_diff(args: &[String]) {
    if args.len() < 2 {
        eprintln!("用法: tautcore-eval diff <baseline.json> <current.json>");
        std::process::exit(1);
    }
    ...
    let baseline: tautcore_eval::EvalReport =
        serde_json::from_str(&baseline_json).unwrap_or_else(|e| {
            eprintln!("错误: 解析基线文件失败: {}", e);
            std::process::exit(1);
        });

    let current: tautcore_eval::EvalReport =
    ...
    let engine = EvalEngine::new();
    let mut diff_report = tautcore_eval::EvalReport::new("差异对比报告");
...
fn cmd_baseline(args: &[String]) {
    if args.is_empty() {
        eprintln!("用法: tautcore-eval baseline <showcase目录> [-o target/showcase-baseline.json]");
        std::process::exit(1);
    }
    ...
    let baseline = tautcore_eval::generate_baseline(showcase_dir).unwrap_or_else(|e| {
        eprintln!("生成失败: {e}");
        std::process::exit(1);
    });
...
fn cmd_baseline_check(args: &[String]) {
    if args.is_empty() {
        eprintln!(
            "用法: tautcore-eval baseline-check <showcase目录> [--baseline target/showcase-baseline.json] [-f text|json] [-o 文件]"
        );
        std::process::exit(1);
    }
    ...
    let baseline = tautcore_eval::ShowcaseBaseline::load(&baseline_path).unwrap_or_else(|e| {
        eprintln!("加载基线失败: {e}");
        eprintln!("提示: 先运行 `tautcore-eval baseline showcase/` 生成基线");
        std::process::exit(1);
    });

    let mut report =
        tautcore_eval::compare_with_baseline(showcase_dir, &baseline).unwrap_or_else(|e| {
            eprintln!("对比失败: {e}");
            std::process::exit(1);
        });
...
fn format_report(report: &tautcore_eval::EvalReport, fmt: &str) -> String {
    match fmt {
        "json" => report.to_json(),
...
fn append_baseline_diff(
    report: &mut tautcore_eval::EvalReport,
    baseline_path: &str,
    engine: &EvalEngine,
) {
    ...
    let baseline: tautcore_eval::EvalReport = match serde_json::from_str(&baseline_json) {
        Ok(r) => r,
        Err(e) => {
...
fn save_to_history(report: &tautcore_eval::EvalReport, label: &str) {
    let history_dir = Path::new(".eval-history");
    if let Ok(store) = tautcore_eval::history::HistoryStore::new(history_dir) {
...
fn get_configs(mode: &str) -> Vec<tautcore_eval::AlgorithmConfig> {
    match mode {
...
fn parse_diagram(source: &str) -> tautcore_core::ast::Diagram {
    try_parse_diagram(source).unwrap_or_else(|| {
        eprintln!("错误: 解析 .taut 文件失败");
        std::process::exit(1);
    })
        }

fn try_parse_diagram(source: &str) -> Option<tautcore_core::ast::Diagram> {
    let raw = tautcore_core::pipeline::parse(source).ok()?;
    let output =
        tautcore_core::pipeline::prepare(raw, &tautcore_core::prepare::StyleRequest::default())
            .ok()?;
