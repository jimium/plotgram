//! 算法性能基准测试
//!
//! 扫描 showcase 目录的真实 .taut 文件，分别评估布局算法和路由算法，
//! 输出 JSON 供可视化脚本使用。
//!
//! 布局和路由是两个独立维度，分开评估：
//! - 布局对比：固定路由为默认，只比布局算法
...
//!
//! 用法:
//!   cargo run -p tautcore-eval --bin bench
//!   cargo run -p tautcore-eval --bin bench -- --output bench_result.json
//!   cargo run -p tautcore-eval --bin bench -- --showcase /path/to/showcase

use tautcore_core::layout;
use tautcore_eval::engine::presets;
use tautcore_eval::engine::EvalEngine;
use tautcore_eval::report::EvalReport;
use std::fs;
use std::path::Path;
...
fn load_showcase(dir: &Path) -> Vec<(String, tautcore_core::ast::Diagram)> {
...
fn main() {
    ...
    if diagrams.is_empty() {
        eprintln!("✗ 没有找到 .taut 文件");
        std::process::exit(1);
    }

    let mut type_counts: std::collections::HashMap<tautcore_core::types::DiagramType, usize> =
