...
//!   cargo run --release -p tautcore-eval --bin atlas_perf
//!   cargo run --release -p tautcore-eval --bin atlas_perf -- --set benchmarks/sets/demo-observe-set.txt
//!   cargo run --release -p tautcore-eval --bin atlas_perf -- --output /tmp/atlas_perf.md
...
fn attach_probe_port(
    ...
    node: &str,
    track: TrackId,
) -> Result<(), tautcore_core::layout::atlas::channel::SubstrateError> {
    let side = match substrate.track(track).map(|t| t.orient) {
        Some(tautcore_core::layout::atlas::channel::TrackOrient::Cross) => PortSide::MainLow,
        _ => PortSide::CrossLow,
    };
    let slot_index = {
