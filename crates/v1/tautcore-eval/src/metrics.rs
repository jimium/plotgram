...
use tautcore_core::layout::quality::refine::segment_intersects_node;
use tautcore_core::layout::geometry::{Point, Rect};
...
/// 线段是否与矩形相交
///
/// 委托到 `tautcore_core::layout::refine::segment_intersects_node`，
...
fn compute_group_gap_deficit(diagram: &Diagram, result: &LayoutResult) -> f64 {
    ...
    let groups: Vec<(&String, &tautcore_core::layout::GroupLayout)> = result.groups.iter().collect();
...
mod tests {
    use super::*;
    use tautcore_core::layout::{PathGeometry, Port};
    ...
    fn sample_layout() -> LayoutResult {
        LayoutResult {
            ...
            groups: tautcore_core::layout::GroupTable::new(),
