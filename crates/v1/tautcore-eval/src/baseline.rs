...
use tautcore_core::layout::{compute_layout_with_plan, LayoutResult, LintMetricsSummary};
use tautcore_core::pipeline::parse_prepare_validate;
...
pub struct ShowcaseBaselineEntry {
    /// 相对 showcase 根目录的路径（如 flowchart/c.payment-flow.taut）
    pub path: String,
...
fn evaluate_pgm_file(path: &Path) -> Result<EvaluatedEntry, String> {
    ...
    let start = tautcore_core::layout::perf::Instant::now();
...
mod tests {
    ...
    fn k8s_architecture_layout_is_deterministic() {
        ...
        let path = root.join("showcase/architecture/demo.k8s-multi-cluster-federation.taut");
    ...
    fn layout_stress_nested_is_deterministic() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
        let path = root.join("showcase/architecture/stress.layout-stress-nested.taut");
