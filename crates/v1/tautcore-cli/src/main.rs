//! Tautcore CLI
//!
//! 命令行工具，用于解析、验证和渲染 Tautcore 文件（.taut）。
//!
//! ## 实验环境变量
//!
//! - `TAUTCORE_ATLAS_PLAN_CACHE=<path>`：**实验特性**（M8）。指向可读写的 Plan JSON
//!   缓存文件，命中时走 `compute_layout_incremental`（可跳过相 I 选路）。
//!   默认渲染路径**不**依赖此变量；无 CI 门禁。槽位拓扑未对齐时静默全量相 I。
//!   生产勿默认打开。

use clap::{Parser, Subcommand};
use tautcore_core::diff2::{self, ChangeSet, ChangeOp};
use tautcore_core::error::{DiagnosticError, TautcoreError};
use tautcore_core::interchange::mindmap::{
    import_interchange, InputFormat, MarkdownImportOptions,
};
use tautcore_core::prepare::StyleRequest;
use tautcore_core::pipeline::{import_prepare_validate, parse_prepare, parse_prepare_validate, PipelineOutput};
use tautcore_core::pipeline::{render_json, render_text};
use tautcore_core::RenderFormat;
use std::fs;

#[derive(Parser)]
#[command(
    name = "tautcore",
    about = "Tautcore - Turn anything into a diagram",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    Render {
        /// 输入的 .taut 文件路径
        input: String,
        /// 输出格式 (svg/ascii/json/drawio/md-outline/opml/freemind)；位图请用 tautcore-raster 转换 SVG
...
fn main() {
    ...
    match cli.command {
        ...
        None => {
            println!("Tautcore - Turn anything into a diagram");
            println!("使用 'tautcore --help' 查看可用命令");
        }
    }
}
...
fn resolve_input_format(cli_format: Option<&str>, input_path: &str) -> InputFormat {
    match cli_format {
        Some(f) => match f {
            ...
            "taut" => InputFormat::Tautcore,
            _ => {
                eprintln!(
                    "错误: 不支持的输入格式 '{}'。支持的格式: taut, md-outline",
                    f
                );
                std::process::exit(1);
            }
...
/// 打印渲染阶段 `TautcoreError` 中的完整诊断信息（含 valid_values 等上下文）。
fn print_render_error(e: TautcoreError, source: Option<&str>) {
...
fn cmd_render(
    ...
    let format = RenderFormat::from_str(format_str).unwrap_or_else(|| {
        eprintln!(
            "错误: 不支持的格式 '{}'。支持的格式: svg, ascii, json, drawio, md-outline, opml, freemind（位图请用 tautcore-raster 将 SVG 转为 PNG/WebP）",
            format_str
        );
        std::process::exit(1);
    });
    ...
    // Render（实验：TAUTCORE_ATLAS_PLAN_CACHE → Plan 增量；SVG 注入 layout）
    let mut request = tautcore_core::render::RenderRequest::new(&prepared, format);
    request.transparent_background = transparent_background;
    request.show_title = show_title;

    let cached_layout = if let Ok(cache) = std::env::var("TAUTCORE_ATLAS_PLAN_CACHE") {
            ...
            match format {
                RenderFormat::Svg => {
                    let output_content = if let Some(ref layout) = cached_layout {
                        tautcore_core::pipeline::render_svg_with_layout(&request, layout.clone())
                            .unwrap_or_else(|e| {
...
fn cmd_validate(input: &str, format_str: &str, layout_check: bool) {
    ...
    match format_str {
        ...
        _ => {
            ...
            // 布局质量检查
            if layout_check {
                let config = tautcore_core::layout::LintConfig::strict();
...
fn build_lint_config(
    ...
    fail_on_warning: bool,
    advice: bool,
) -> tautcore_core::layout::LintConfig {
    let profile = tautcore_core::layout::parse_lint_profile(profile).unwrap_or_else(|| {
        eprintln!("未知 lint profile '{profile}'，使用 default（可选：default / strict / ci / verbose / all）");
        tautcore_core::layout::LintProfile::Default
    });
    let mut config = tautcore_core::layout::LintConfig::profile(profile);
    if let Some(ignore_list) = ignore {
        let rules = tautcore_core::layout::parse_lint_rules_list(ignore_list);
...
fn run_layout_lint(
    prepared: &tautcore_core::ast::PreparedDiagram,
    config: &tautcore_core::layout::LintConfig,
) -> Result<tautcore_core::layout::LintReport, tautcore_core::error::DiagnosticError> {
    let diagram = prepared.inner();
    let layout = if let Ok(cache) = std::env::var("TAUTCORE_ATLAS_PLAN_CACHE") {
        layout_with_optional_plan_cache(diagram, prepared.layout_plan(), &cache)?
    } else {
        tautcore_core::layout::compute_layout_with_plan(diagram, prepared.layout_plan())?
    };
    Ok(tautcore_core::layout::LayoutLinter::with_config(config.clone()).run(diagram, &layout))
}

fn layout_with_optional_plan_cache(
    diagram: &tautcore_core::ast::Diagram,
    plan: &tautcore_core::layout::pipeline::plan::LayoutPlan,
    cache_path: &str,
) -> Result<tautcore_core::layout::LayoutResult, tautcore_core::error::DiagnosticError> {
    let path = std::path::Path::new(cache_path);
    if path.is_file() {
        if let Ok(bytes) = std::fs::read(path) {
            if let Ok(prev) =
                serde_json::from_slice::<tautcore_core::layout::atlas::plan::Plan>(&bytes)
            {
                eprintln!(
                    "[atlas] experimental TAUTCORE_ATLAS_PLAN_CACHE → incremental"
                );
                let layout =
                    tautcore_core::layout::compute_layout_incremental(diagram, &prev)?;
    ...
    let layout = tautcore_core::layout::compute_layout_with_plan(diagram, plan)?;
    if let Some(p) = layout.hints.atlas_plan.as_ref() {
        let _ = std::fs::write(path, serde_json::to_vec_pretty(p.as_ref()).unwrap_or_default());
    }
    Ok(layout)
...
fn print_lint_report_text(
    report: &tautcore_core::layout::LintReport,
    config: &tautcore_core::layout::LintConfig,
    ...
    for (idx, v) in report.violations.iter().enumerate() {
        let level = match v.severity {
            tautcore_core::layout::LintSeverity::Error => "error",
            tautcore_core::layout::LintSeverity::Warning => "warning",
        };
        let metric = v
            .metric
            .map(|m| format!(" ({m:.1})"))
...
fn cmd_diff(old_file: &str, new_file: &str, format_str: &str) {
    ...
    let old_diagram = tautcore_core::parse(&old_source).unwrap_or_else(|e| {
        for err in e.into_diagnostics() {
            eprintln!("{}", err);
        }
        std::process::exit(1);
    });
    let new_diagram = tautcore_core::parse(&new_source).unwrap_or_else(|e| {
        for err in e.into_diagnostics() {
            eprintln!("{}", err);
        }
        std::process::exit(1);
...
fn cmd_patch(input: &str, patch_file: &str, output: Option<&str>) {
    ...
    let raw_diagram = tautcore_core::parse(&source).unwrap_or_else(|e| {
        for err in e.into_diagnostics() {
            eprintln!("{}", err);
        }
        std::process::exit(1);
    ...
    let prepare_output = match tautcore_core::pipeline::prepare(result.diagram, &StyleRequest::default()) {
        Ok(output) => output,
        Err(e) => {
            for err in e.into_diagnostics() {
                eprintln!("{}", err);
