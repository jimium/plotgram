...
impl DiagnosticError {
    ...
    ///
    /// LSP 行列从 0 开始，Tautcore 从 1 开始，此处自动转换。
...
// ═══════════════════════════════════════════════════════════════════════
// TautcoreError（内部错误类型）
// ═══════════════════════════════════════════════════════════════════════

/// Tautcore 内部错误（用于 Rust 层面 `Result` 传播）
#[derive(Debug)]
...
impl fmt::Display for TautcoreError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl std::error::Error for TautcoreError {}

pub type Result<T> = std::result::Result<T, TautcoreError>;
