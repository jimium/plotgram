...
pub enum EntityIdStrategy {
    ...
    Slugify,
    /// 解析 Export 写入的 <!-- tautcore:entity-id -->
    FromMetadata,
}
...
fn parse_atx_outline(
    ...
    while i < lines.len() {
        match &lines[i].kind {
            ...
            LineKind::HtmlComment(content) => {
                // 解析 <!-- tautcore:entity-id=xxx -->
                if let Some(entity_id) = parse_tautcore_entity_id(content) {
...
/// 解析 <!-- tautcore:entity-id=xxx --> 注释。
fn parse_tautcore_entity_id(content: &str) -> Option<String> {
    let content = content.trim();
    if let Some(rest) = content.strip_prefix("tautcore:entity-id=") {
