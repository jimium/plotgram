...
//!
//! 用法:
//!   cargo run --release -p tautcore-core --bin congestion-baseline -- \
//!     [--set FILE] [--date YYYY-MM-DD] [--calibrate] [file.taut ...]
//!
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "用法: congestion-baseline [--set FILE] [--date YYYY-MM-DD] [--calibrate] [file.taut ...]\n\
    ...
    if files.is_empty() {
        let candidates = [
            set_file.clone(),
            Some(PathBuf::from("benchmarks/sets/mech-set.txt")),
        ];
    ...
    if files.is_empty() {
        die("无输入文件（检查 --set 或 benchmarks/sets/mech-set.txt）");
    }
