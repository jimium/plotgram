...
impl<'a, 's> DrawioEncoder<'a, 's> {
    ...
    fn check_unsupported(&mut self) -> Result<()> {
        ...
        if !self.options.allow_unsupported_diagram_types {
            return Err(crate::error::TautcoreError::render_internal_msg(format!(
        ...
        match self.options.fallback {
            DrawioFallback::Error => Err(crate::error::TautcoreError::render_internal_msg(
            ...
            DrawioFallback::EmbeddedSvg => {
                ...
                Err(crate::error::TautcoreError::render_internal_msg(
...
<mxfile host="Tautcore/{crate_version}" agent="Tautcore/{crate_version}" version="1.0">
  <diagram id="tautcore-diagram" name="{title}">
    ...
    fn write_groups(&mut self) {
        ...
        for (gid, group, abs_x, abs_y, has_label, parent_gid) in group_entries {
            ...
            if self.options.include_export_metadata {
                style_parts.push(format!("tautcoreGroupId={}", sanitize_id(&gid)));
            }
...
"#,
    ...
    fn write_edge(&mut self, edge: &ExportEdge<'_>) {
        ...
        // 元数据
        if self.options.include_export_metadata {
            style_parts.push(format!("tautcoreRelationIndex={}", edge.index));
            style_parts.push(format!("tautcoreDegrade={}", routing.tier.as_str()));
        }
        let style = style_parts.join(";");
...
"#,
    ...
    fn write_node(&mut self, node: &ExportNode<'_>) {
        ...
        if self.options.include_export_metadata {
            style_parts.push(format!("tautcoreEntityId={}", entity_id));
            if tier > DegradeTier::L0 {
                style_parts.push(format!("tautcoreDegrade={}", tier.as_str()));
            }
        }
        let style = style_parts.join(";");
