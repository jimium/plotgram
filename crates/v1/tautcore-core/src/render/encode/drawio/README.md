...
- UTF-8 XML，根元素 `<mxfile host="Tautcore/{version}">`
...
- `tautcoreEntityId` — 节点
- `tautcoreRelationIndex` — 边
- `tautcoreGroupId` — 分组
- `tautcoreDegrade` — 该元素降级级别
...
| `include_export_metadata` | `true` | tautcore* 元数据 |
...
集成测试：`crates/tautcore-core/tests/drawio_export.rs`（全管线 + XML 结构、颜色、箭头、端口等断言）。
