...
mod tests {
    ...
    fn svg_includes_attribution_by_default() {
        ...
        assert!(svg.contains("class=\"tautcore-attribution\""));
        assert!(svg.contains("by "));
        assert!(svg.contains("Tautcore"));
    ...
    fn svg_attribution_can_be_disabled() {
        ...
        let svg = encode(&request).unwrap();
        assert!(!svg.contains("tautcore-attribution"));
        assert!(!svg.contains("by "));
        assert!(!svg.contains("Tautcore"));
    }
    ...
    fn er_user_post_svg_keeps_table_nodes_in_viewbox() {
        let source = include_str!("../../../../../showcase/er/smoke.user-post.taut");
