...
use crate::error::{DiagnosticError, TautcoreError, Result};
...
/// 解析 Tautcore 源文本为 Diagram AST
///
/// 始终走 fallback 路径以收集尽可能多的错误（spec §6.1）。
/// 如果存在任何错误，返回 `Err(TautcoreError::Parse(all_errors))`。
pub fn parse(source: &str) -> Result<Diagram> {
    let (diagram, errors, _warnings) = parse_with_diagnostics(source);
    if errors.is_empty() {
        diagram.ok_or_else(|| TautcoreError::parse_msg("no diagram produced"))
    } else {
        Err(TautcoreError::Parse(errors))
    }
}
...
impl Parser {
    ...
    fn parse_file(&mut self) -> Result<Diagram> {
        ...
        if self.at_eof() {
            ...
            return Err(TautcoreError::parse_msg("missing diagram declaration"));
        }
        ...
        if !matches!(self.peek_kind(), TokenKind::Diagram) {
            ...
            return Err(TautcoreError::parse_msg("expected 'diagram'"));
        }
        self.advance();
        ...
        if !self.expect_lbrace() {
            return Err(TautcoreError::parse_msg("expected '{'"));
        }
        ...
        if !self.errors.is_empty() {
            return Err(TautcoreError::Parse(self.errors.clone()));
        }
    ...
    fn parse_diagram_type(&mut self) -> Result<DiagramType> {
        ...
        let dt = match self.peek_kind() {
            ...
            _ => {
                ...
                return Err(TautcoreError::parse_msg("invalid diagram type"));
            }
        };
        self.advance();
        Ok(dt)
