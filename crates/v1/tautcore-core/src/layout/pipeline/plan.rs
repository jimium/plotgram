...
///
/// 解析优先级：diagram attr `pipeline:` > 环境变量 `TAUTCORE_PIPELINE` >
...
impl PipelineChoice {
    ...
    fn resolve(diagram: &Diagram) -> Self {
        ...
        // 2. 环境变量次之
        if let Ok(v) = std::env::var("TAUTCORE_PIPELINE") {
            if let Some(choice) = Self::parse(&v) {
                return choice;
            }
            crate::perf_log!(
                "[pipeline] unknown TAUTCORE_PIPELINE={v:?} (shadow retired); falling back to atlas"
            );
        }
        // 3. 默认 Atlas
        Self::Atlas
...
mod tests {
    ...
    fn pipeline_choice_resolution_priority() {
        std::env::remove_var("TAUTCORE_PIPELINE");
        ...
        std::env::set_var("TAUTCORE_PIPELINE", "shadow");
        assert_eq!(
            PipelineChoice::resolve(&Diagram::default()),
            PipelineChoice::Atlas,
        ...
        std::env::set_var("TAUTCORE_PIPELINE", "legacy");
        assert_eq!(
            PipelineChoice::resolve(&Diagram::default()),
            PipelineChoice::Atlas,
        ...
        std::env::set_var("TAUTCORE_PIPELINE", "bogus");
        assert_eq!(
            PipelineChoice::resolve(&Diagram::default()),
            PipelineChoice::Atlas,
