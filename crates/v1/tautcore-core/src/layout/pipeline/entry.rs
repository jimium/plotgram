...
/// - "flowchart": 流程图分层布局
/// - "er": ER 图分层布局
/// - "state": 状态图布局（Sugiyama / circular 双路径）
/// - "architecture": 架构图布局（分组感知两阶段）
/// - "mindmap": 思维导图布局（中心辐射 / 单向树）
/// - "sequence": 时序图布局
/// - "auto": 元选择器，解析为当前图类型的 profile 默认算法
///
...
///
/// **实验特性（M8）**：供 CLI `TAUTCORE_ATLAS_PLAN_CACHE` 等可选入口使用；
...
pub(super) fn all_layout_strategies() -> Vec<Box<dyn LayoutStrategy>> {
    crate::layout::pipeline::registry::all_layout_strategies()
}
...
pub(crate) fn known_layout_algo_names() -> Vec<&'static str> {
    crate::layout::pipeline::registry::LAYOUT_ALGORITHM_NAMES.to_vec()
}

pub(crate) fn known_edge_routing_names() -> Vec<&'static str> {
    crate::layout::pipeline::registry::EDGE_ROUTING_NAMES.to_vec()
}
...
fn validate_direction_layout_compat(
    ...
    let strategy = crate::layout::pipeline::registry::build_layout_strategy(algo, &plan)
...
fn validate_registered_layout_algo(
    ...
    // "auto" 对所有图类型合法，解析为 profile 默认算法
    if value == "auto" {
        return Ok(());
    }
