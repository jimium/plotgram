...
mod tests {
    ...
    #[test]
    fn bezier_controls_respect_ports() {
        let vertical = compute_bezier_controls(
            100.0, 0.0, 100.0, 100.0, Port::Bottom, Port::Top, DEFAULT_BEZIER_TENSION,
        );
        assert!(vertical[0].y > 0.0);
        assert!(vertical[1].y < 100.0);

        let corner = compute_bezier_controls(
            0.0, 0.0, 100.0, 100.0, Port::Bottom, Port::Left, DEFAULT_BEZIER_TENSION,
        );
        for cp in &corner {
            assert!(cp.x.abs() < 200.0);
            assert!(cp.y.abs() < 200.0);
        }
    }

    #[test]
    fn point_at_path_t_cases() {
        let cases: &[(&[Point], f64, Point)] = &[
            (&[], 0.5, Point::new(0.0, 0.0)),
            (&[Point::new(10.0, 20.0)], 0.5, Point::new(10.0, 20.0)),
            (&[Point::new(0.0, 0.0), Point::new(10.0, 0.0)], 0.0, Point::new(0.0, 0.0)),
            (&[Point::new(0.0, 0.0), Point::new(10.0, 0.0)], 1.0, Point::new(10.0, 0.0)),
            (&[Point::new(0.0, 0.0), Point::new(10.0, 0.0)], 0.5, Point::new(5.0, 0.0)),
            (&[Point::new(0.0, 0.0), Point::new(10.0, 0.0)], -1.0, Point::new(0.0, 0.0)),
            (&[Point::new(0.0, 0.0), Point::new(10.0, 0.0)], 2.0, Point::new(10.0, 0.0)),
            (&[Point::new(5.0, 5.0), Point::new(5.0, 5.0)], 0.5, Point::new(5.0, 5.0)),
        ];
        for (path, t, expected) in cases {
            assert_eq!(point_at_path_t(path, *t), *expected, "path={path:?} t={t}");
        }
        ...
        assert!((q1.x - 5.0).abs() < 1e-9 && (q1.y - 0.0).abs() < 1e-9);
        let q3 = point_at_path_t(&path, 0.75);
        assert!((q3.x - 10.0).abs() < 1e-9 && (q3.y - 5.0).abs() < 1e-9);

        let unequal = [Point::new(0.0, 0.0), Point::new(10.0, 0.0), Point::new(10.0, 30.0)];
        let p25 = point_at_path_t(&unequal, 0.25);
        assert!((p25.x - 10.0).abs() < 1e-9 && (p25.y - 0.0).abs() < 1e-9);
        let p50 = point_at_path_t(&unequal, 0.5);
        assert!((p50.x - 10.0).abs() < 1e-9 && (p50.y - 10.0).abs() < 1e-9);
    }
    ...
    #[test]
    fn parse_label_t_cases() {
        let cases: &[(Option<AttributeValue>, f64)] = &[
            (None, 0.5),
            (
                Some(AttributeValue::String(TextValue::quoted("middle"))),
                0.5,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("start"))),
                0.15,
            ),
            (
                Some(AttributeValue::String(TextValue::unquoted("end"))),
                0.85,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("t:0.25"))),
                0.25,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("t:1.5"))),
                1.0,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("t:-0.3"))),
                0.0,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("t:abc"))),
                0.5,
            ),
            (
                Some(AttributeValue::String(TextValue::quoted("weird"))),
                0.5,
            ),
            (
                Some(AttributeValue::String(TextValue::unquoted("START"))),
                0.15,
            ),
        ];
        for (value, expected) in cases {
            let rel = relation_with_label_position(value.clone());
            assert!(
                (parse_label_t(&rel) - expected).abs() < 1e-9,
                "value={value:?}"
            );
        }
    }
    ...
    #[test]
    fn line_style_signature_cases() {
        let cases: &[(&[(&str, AttributeValue)], &str)] = &[
            (&[], "solid"),
            (&[("dashed", AttributeValue::Boolean(true))], "dashed"),
            (&[("dashed", AttributeValue::Boolean(false))], "solid"),
            (
                &[(
                    "stroke_dasharray",
                    AttributeValue::String(TextValue::quoted("8,4")),
                )],
                "dash:8,4",
            ),
            (
                &[(
                    "stroke_dasharray",
                    AttributeValue::String(TextValue::unquoted("2,4")),
                )],
                "dash:2,4",
            ),
            (
                &[
                    (
                        "stroke_dasharray",
                        AttributeValue::String(TextValue::quoted("")),
                    ),
        ...
        for (style, expected) in cases {
            let rel = relation_with_style(style);
            assert_eq!(edge_line_style_signature(&rel), *expected);
        }
        let a = relation_with_style(&[(
            "stroke_dasharray",
            AttributeValue::String(TextValue::quoted("8,4")),
        )]);
        let b = relation_with_style(&[(
            "stroke_dasharray",
            AttributeValue::String(TextValue::quoted("2,4")),
        )]);
        assert_ne!(edge_line_style_signature(&a), edge_line_style_signature(&b));
    }

    #[test]
    fn closest_point_on_path_cases() {
        let empty = closest_point_on_path(&[], Point::new(5.0, 5.0));
        assert_eq!(empty.0, Point::new(0.0, 0.0));
        assert!(empty.1.is_infinite());

        let single = [Point::new(3.0, 4.0)];
        let (pt, dist) = closest_point_on_path(&single, Point::new(0.0, 0.0));
        assert_eq!(pt, Point::new(3.0, 4.0));
        assert!((dist - 5.0).abs() < 1e-9);

        let segment = [Point::new(0.0, 0.0), Point::new(10.0, 0.0)];
        let (on_seg, dist_on) = closest_point_on_path(&segment, Point::new(5.0, 0.0));
        assert_eq!(on_seg, Point::new(5.0, 0.0));
        assert!(dist_on.abs() < 1e-9);

        let (perp, dist_perp) = closest_point_on_path(&segment, Point::new(5.0, 3.0));
        assert!((perp.x - 5.0).abs() < 1e-9 && (perp.y - 0.0).abs() < 1e-9);
        assert!((dist_perp - 3.0).abs() < 1e-9);

        let (clamped, dist_clamped) = closest_point_on_path(&segment, Point::new(-5.0, 5.0));
        assert_eq!(clamped, Point::new(0.0, 0.0));
        assert!((dist_clamped - (50.0_f64).sqrt()).abs() < 1e-9);

        let polyline = [Point::new(0.0, 0.0), Point::new(10.0, 0.0), Point::new(10.0, 10.0)];
        let (corner_pt, corner_dist) = closest_point_on_path(&polyline, Point::new(5.0, 5.0));
        assert!((corner_dist - 5.0).abs() < 1e-9);
        let on_seg1 = corner_pt.y.abs() < 1e-9 && corner_pt.x >= 0.0 && corner_pt.x <= 10.0;
        let on_seg2 = (corner_pt.x - 10.0).abs() < 1e-9 && corner_pt.y >= 0.0 && corner_pt.y <= 10.0;
        assert!(on_seg1 || on_seg2, "pt={corner_pt:?}");

        let (nearest, nearest_dist) = closest_point_on_path(&polyline, Point::new(12.0, 5.0));
        assert!((nearest.x - 10.0).abs() < 1e-9 && (nearest.y - 5.0).abs() < 1e-9);
        assert!((nearest_dist - 2.0).abs() < 1e-9);
    }
    ...
    #[test]
    fn build_edge_labels_cases() {
        let straight_path = [Point::new(0.0, 0.0), Point::new(100.0, 0.0)];
        let point_at = |t: f64| point_at_path_t(&straight_path, t);

        let empty = relation_with_labels(None, None, None);
        assert!(build_edge_labels(&empty, 0.5, Point::new(0.0, 0.0), point_at).is_empty());

        let middle = relation_with_labels(Some("mid"), None, None);
        let middle_labels = build_edge_labels(&middle, 0.5, Point::new(0.0, 5.0), point_at);
        assert_eq!(middle_labels.len(), 1);
        assert_eq!(middle_labels[0].text, "mid");
        assert!((middle_labels[0].center.x - 50.0).abs() < 1e-9);
        assert!((middle_labels[0].center.y - 5.0).abs() < 1e-9);

        let all = relation_with_labels(Some("mid"), Some("head"), Some("tail"));
        let all_labels = build_edge_labels(&all, 0.5, Point::new(0.0, 0.0), point_at);
        assert_eq!(all_labels.len(), 3);
        assert_eq!(all_labels[0].text, "mid");
        assert_eq!(all_labels[1].text, "tail");
        assert_eq!(all_labels[2].text, "head");
        assert!((all_labels[0].center.x - 50.0).abs() < 1e-9);
        assert!((all_labels[1].center.x - 15.0).abs() < 1e-9);
        assert!((all_labels[2].center.x - 85.0).abs() < 1e-9);

        let ends = relation_with_labels(None, Some("H"), Some("T"));
        let end_labels = build_edge_labels(&ends, 0.5, Point::new(0.0, 0.0), point_at);
        assert_eq!(end_labels.len(), 2);
        assert_eq!(end_labels[0].text, "T");
        assert_eq!(end_labels[1].text, "H");

        let custom_t = relation_with_labels(Some("mid"), None, None);
        let custom_labels = build_edge_labels(&custom_t, 0.25, Point::new(0.0, 0.0), point_at);
        assert_eq!(custom_labels.len(), 1);
        assert!((custom_labels[0].center.x - 25.0).abs() < 1e-9);

        let offset_labels = build_edge_labels(&all, 0.5, Point::new(10.0, -5.0), point_at);
        assert_eq!(offset_labels.len(), 3);
        assert!((offset_labels[0].center.x - 60.0).abs() < 1e-9);
        assert!((offset_labels[0].center.y - (-5.0)).abs() < 1e-9);
        assert!((offset_labels[1].center.x - 25.0).abs() < 1e-9);
        assert!((offset_labels[2].center.x - 95.0).abs() < 1e-9);

        let bezier_labels = build_edge_labels(&all, 0.5, Point::new(0.0, 0.0), |t| {
            Point::new((1.0 - t) * 0.0 + t * 100.0, (1.0 - t) * 0.0 + t * 100.0)
        });
        assert_eq!(bezier_labels.len(), 3);
        assert!((bezier_labels[0].center.x - 50.0).abs() < 1e-9);
        assert!((bezier_labels[0].center.y - 50.0).abs() < 1e-9);
        assert!((bezier_labels[1].center.x - 15.0).abs() < 1e-9);
        assert!((bezier_labels[1].center.y - 15.0).abs() < 1e-9);
        assert!((bezier_labels[2].center.x - 85.0).abs() < 1e-9);
        assert!((bezier_labels[2].center.y - 85.0).abs() < 1e-9);
    }

    #[test]
    fn build_edge_labels_rotation_cases() {
        let horizontal = relation_with_labels(Some("mid"), Some("head"), Some("tail"));
        let h_labels = build_edge_labels(&horizontal, 0.5, Point::new(0.0, 0.0), |t| {
            point_at_path_t(&[Point::new(0.0, 0.0), Point::new(100.0, 0.0)], t)
        });
        assert_eq!(h_labels.len(), 3);
        for lbl in &h_labels {
            assert!(lbl.rotation.abs() < 1e-6, "got {}", lbl.rotation);
        }

        let vertical = relation_with_labels(Some("mid"), None, None);
        let v_labels = build_edge_labels(&vertical, 0.5, Point::new(0.0, 0.0), |t| {
            point_at_path_t(&[Point::new(0.0, 0.0), Point::new(0.0, 100.0)], t)
        });
        assert_eq!(v_labels.len(), 1);
        assert!((v_labels[0].rotation - 90.0).abs() < 1e-6);

        let diagonal = relation_with_labels(Some("mid"), None, None);
        let d_labels = build_edge_labels(&diagonal, 0.5, Point::new(0.0, 0.0), |t| {
            point_at_path_t(&[Point::new(0.0, 0.0), Point::new(100.0, 100.0)], t)
        });
        assert_eq!(d_labels.len(), 1);
        assert!((d_labels[0].rotation - 45.0).abs() < 1e-6);
...
mod organic_tests {
    ...
    #[test]
    fn organic_bezier_control_cases() {
        let horizontal = compute_bezier_controls_organic(
            0.0, 0.0, 100.0, 0.0,
            Port::Right, Port::Left,
            0.55, DEFAULT_SHOULDER_RATIO,
        );
        assert!(horizontal[0].x > 0.0);
        assert!(horizontal[1].x < 100.0);
        assert!(horizontal[0].x < 100.0);
        assert!(horizontal[1].x > 0.0);

        let diagonal = compute_bezier_controls_organic(
            0.0, 0.0, 100.0, 100.0,
            Port::Right, Port::Left,
            0.55, DEFAULT_SHOULDER_RATIO,
        );
        let dist = 141.42;
        assert!(diagonal[0].x >= 0.0);
        assert!(diagonal[0].y >= -dist * 0.5);
        assert!(diagonal[1].x <= 100.0 + dist * 0.5);
        assert!(diagonal[1].y <= 100.0 + dist * 0.5);
        let cp1_ext = ((diagonal[0].x).powi(2) + (diagonal[0].y).powi(2)).sqrt();
        assert!(cp1_ext > 0.0);
        ...
        assert!(cp1_hi.x > cp1_lo.x);

        let [cp1_low_tension, _] = compute_bezier_controls_organic(
            0.0, 0.0, 100.0, 0.0,
            Port::Right, Port::Left,
            0.01, DEFAULT_SHOULDER_RATIO,
        );
        let low_ext = ((cp1_low_tension.x).powi(2) + (cp1_low_tension.y).powi(2)).sqrt();
        assert!(low_ext < 30.0);

        let [cp1_short, _] = compute_bezier_controls_organic(
            0.0, 0.0, 5.0, 0.0,
            Port::Right, Port::Left,
            0.1, DEFAULT_SHOULDER_RATIO,
        );
        let short_ext = ((cp1_short.x).powi(2) + (cp1_short.y).powi(2)).sqrt();
        assert!(short_ext > 0.0 && short_ext <= 5.0);
        ...
        assert!(cp1_large.x > cp1_small.x);
    }

    #[test]
    fn organic_preserves_tangents_and_radial_bloom() {
        let [cp1, cp2] = compute_bezier_controls_organic(
            0.0, 10.0, 200.0, 80.0,
            Port::Right, Port::Left,
            0.8, DEFAULT_SHOULDER_RATIO,
        );
        assert!((cp1.y - 10.0).abs() < 1e-9, "cp1 y={}", cp1.y);
        assert!((cp2.y - 80.0).abs() < 1e-9, "cp2 y={}", cp2.y);
        assert!(cp1.x > 0.0);
        assert!(cp2.x < 200.0);
        assert!((cp1.x - 0.0) > (200.0 - cp2.x));

        let len = (0.6_f64.powi(2) + 0.8_f64.powi(2)).sqrt();
        let from_dir = Point::new(0.6 / len, 0.8 / len);
        let to_dir = Point::new(-1.0, 0.0);
        let [radial_cp1, radial_cp2] = compute_bezier_controls_organic_tangents(
            50.0, 50.0, 200.0, 120.0,
            from_dir, to_dir, 0.7, DEFAULT_SHOULDER_RATIO,
        );
        let vx = radial_cp1.x - 50.0;
        let vy = radial_cp1.y - 50.0;
        let cross = (vx * from_dir.y - vy * from_dir.x).abs();
        assert!(cross < 1e-6, "cross={cross}");
        assert!((radial_cp2.y - 120.0).abs() < 1e-9);
    }
}
