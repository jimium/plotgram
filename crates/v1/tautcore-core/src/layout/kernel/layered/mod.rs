...
//!
//! 本模块为内部分层布局引擎，不直接暴露为用户可选算法。
//! `flowchart` / `er` / `state` 等图类型配方通过 [`engine::compute_with_preset`] 与
//! [`layered_kernel::LayeredKernel`] 调用本引擎，各自使用不同的 [`preset`] 参数集。

pub mod engine;
pub mod preset;
pub(in crate::layout) mod layered_kernel;

pub(in crate::layout) mod graph;
pub(in crate::layout) mod rank;
pub(in crate::layout) mod order;
pub(in crate::layout) mod coordinate;
pub(in crate::layout) mod postprocess;
...
mod tests {
    ...
    fn v2_layout_builds_nodes() {
        ...
        let result = engine::compute_with_preset(&diagram, &preset::GENERIC_PRESET, SugiyamaLayoutConfig::default());
    ...
    fn pipeline_layout_signature(source: &str) -> Vec<(String, i64, i64, i64, i64)> {
        ...
        let result = engine::compute_with_preset(prepared.inner(), &preset::GENERIC_PRESET, SugiyamaLayoutConfig::default());
    ...
    fn v3b_user_auth_pendants_pack_under_auth() {
        let source = include_str!("../../../../../../showcase/flowchart/product.user-auth.taut");
