//! tautcore-raster CLI:将 SVG 文件转为 PNG / WebP。
//!
//! 用法:
//! ```text
//! tautcore-raster input.svg -o out.png            # 格式按 -o 扩展名推断
//! tautcore-raster input.svg -o out.bin -f webp    # --format 覆盖推断
//! cat input.svg | tautcore-raster - -o out.png    # 从 stdin 读入
//! ```
...
use clap::Parser;
use tautcore_raster::{rasterize_svg_to_file, RasterFormat, RasterOptions};

#[derive(Parser)]
#[command(name = "tautcore-raster", about = "将 SVG 转为 PNG / WebP 位图", version)]
struct Cli {
    ...
    /// 字体文件目录(覆盖 TAUTCORE_FONTS_DIR 环境变量;均未设置时使用 cwd/fonts/)
    #[arg(long = "fonts-dir")]
