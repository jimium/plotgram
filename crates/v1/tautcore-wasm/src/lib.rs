...
pub struct RenderResult {
    ...
    pub export_report: Option<tautcore_core::render::encode::ExportReport>,
}
...
pub struct LintResult {
    pub success: bool,
    pub acceptable: bool,
    pub report: tautcore_core::layout::LintReport,
    pub errors: Vec<DiagnosticError>,
    pub warnings: Vec<DiagnosticError>,
}
...
pub struct WasmRenderOptions {
    ...
    pub ascii: Option<tautcore_core::render::encode::ascii::AsciiExportOptions>,
}
...
pub fn layout_catalog() -> String {
    serde_json::to_string(&tautcore_core::layout::layout_catalog())
...
fn render_from_pipeline(
    output: tautcore_core::pipeline::PipelineOutput,
    format: RenderFormat,
    format_str: &str,
    ...
    match serde_json::from_str::<WasmLintOptions>(options_json) {
        ...
        Err(err) => {
            let result = LintResult {
                success: false,
                acceptable: false,
                report: tautcore_core::layout::LintReport::default(),
                errors: vec![DiagnosticError::render_internal(
                    tautcore_core::ast::Span::dummy(),
...
fn lint_impl(source: &str, options: Option<WasmLintOptions>) -> String {
    ...
    let mut report = tautcore_core::layout::LintReport::default();
    let mut success = false;
    let mut acceptable = false;
    if let Some(prepared) = output.diagram {
        if errors.is_empty() {
            let diagram = prepared.inner();
            match tautcore_core::layout::compute_layout_with_plan(diagram, prepared.layout_plan()) {
                Ok(layout) => {
                    report = tautcore_core::layout::LayoutLinter::with_config(config.clone())
                        .run(diagram, &layout);
                    success = true;
