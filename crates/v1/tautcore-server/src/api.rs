//! Tautcore Server HTTP API handlers
...
static HEADER_FORMAT: HeaderName = HeaderName::from_static("x-tautcore-format");
static HEADER_VALID: HeaderName = HeaderName::from_static("x-tautcore-valid");
static HEADER_WARNINGS: HeaderName = HeaderName::from_static("x-tautcore-warnings");
...
/// 解析 + prepare + 校验 Tautcore 源码，返回 PreparedDiagram 与结构化诊断结果。
fn check_source(source: &str) -> (Option<PreparedDiagram>, CheckResult) {
    let output = tautcore_core::pipeline::parse_prepare_validate(source, &StyleRequest::default());
