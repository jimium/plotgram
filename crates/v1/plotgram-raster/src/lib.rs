//! Plotgram SVG 栅格化库:将 SVG 字符串转为 PNG / WebP 位图。
//!
//! 独立于 plotgram-core 的叶子 crate,输入就是 SVG 字符串,
//! 通过 usvg/resvg 栅格化为像素,再由 image crate 编码为目标格式。
//!
//! 字体目录解析优先级(高 → 低):
//! 1. [`RasterOptions::fonts_dir`](如 CLI `--fonts-dir`)
//! 2. 环境变量 [`FONTS_DIR_ENV_VAR`](`PLOTGRAM_FONTS_DIR`)
//! 3. 当前工作目录下的 `fonts/`
...
/// 环境变量名,用于指定字体文件目录
pub const FONTS_DIR_ENV_VAR: &str = "PLOTGRAM_FONTS_DIR";
...
pub struct RasterOptions {
    /// 自定义字体目录(覆盖 `PLOTGRAM_FONTS_DIR` 环境变量与 `cwd/fonts/`)
