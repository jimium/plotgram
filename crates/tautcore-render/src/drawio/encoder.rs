...
use tautcore_model::graph::{Group, NodeRole};
use tautcore_model::render::RenderInput;
use tautcore_model::result::{EdgePlacement, LabelOwner, LabelSlot, NodePlacement};
...
<mxfile host="Tautcore/{version}" agent="Tautcore/{version}" version="1.0">
  <diagram id="tautcore-diagram" name="{name}">
...
"#,
    ...
    fn write_edge(&mut self, ep: &EdgePlacement) {
        ...
        let arrow = graph_edge.map(|e| e.arrow).unwrap_or(tautcore_model::graph::Arrow::Forward);
...
"#,
    ...
    fn format_edge_geometry(
        &self,
        ep: &EdgePlacement,
        waypoints: &[tautcore_model::geometry::Point],
        label_slot: &Option<LabelSlot>,
        endpoints: Option<(tautcore_model::geometry::Point, tautcore_model::geometry::Point)>,
    ) -> String {
