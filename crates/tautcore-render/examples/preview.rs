...
//!
//! Run: `cargo run -p tautcore-render --example preview`
...
use tautcore_model::render::{RenderInput, RenderMeta};
...
use tautcore_render::render_svg;

fn node(
    id: &str,
    label: &str,
    shape: Option<tautcore_model::NodeShape>,
    variant: Option<&str>,
) -> Node {
...
fn build_input(theme: Option<&str>, render_style: Option<&str>) -> RenderInput {
    ...
    let mut graph = graph;
    graph.groups.push(tautcore_model::graph::Group {
        id: "backend".to_string(),
