...
use tautcore_model::graph::{Arrow, NodeRole};
use tautcore_model::port::Side;
use tautcore_parse::{parse, ParseOutput};
...
fn node<'a>(out: &'a ParseOutput, id: &str) -> &'a tautcore_model::graph::Node {
    out.graph
        .find_node(id)
        .unwrap_or_else(|| panic!("node `{id}` not found"))
}

fn edge_at(out: &ParseOutput, idx: usize) -> &tautcore_model::graph::Edge {
    &out.graph.edges[idx]
}
...
fn node_sugar_conflict_label() {
    ...
    assert!(
        matches!(&err, tautcore_parse::ParseError::DuplicateAttr { key, .. } if key == "label"),
...
fn node_sugar_conflict_archetype() {
    ...
    assert!(
        matches!(&err, tautcore_parse::ParseError::DuplicateAttr { key, .. } if key == "archetype"),
...
fn node_sugar_conflict_icon() {
    ...
    assert!(
        matches!(&err, tautcore_parse::ParseError::DuplicateAttr { key, .. } if key == "icon"),
...
fn node_sugar_archetype_expands_axes() {
    ...
    let n = node(&out, "db");
    assert_eq!(n.shape, Some(tautcore_model::NodeShape::Cylinder));
    assert_eq!(
        n.attrs.get("variant").and_then(|v| v.as_str()),
        Some("info")
    );
...
fn node_sugar_icon_blocks_archetype_icon() {
    ...
    assert_eq!(n.shape, Some(tautcore_model::NodeShape::RoundedRect));
    assert_eq!(
        n.attrs.get("variant").and_then(|v| v.as_str()),
        Some("default")
    );
...
fn diagram_profile_plus_layout_override() {
    ...
    assert_eq!(
        out.profile,
        Some(tautcore_model::profile::DiagramType::Sequence)
    );
...
fn diagram_meta_namespace_attrs() {
    ...
    assert_eq!(
        ...
        Some(&tautcore_model::attr::AttrValue::Str("jane".into()))
    );
...
fn archetype_expansion_all_builtin() {
    use tautcore_model::archetype::ARCHETYPES;
...
fn archetype_fill_only_never_overrides() {
    ...
    let n = node(&out, "n");
    assert_eq!(n.shape, Some(tautcore_model::NodeShape::Circle));
    assert_eq!(
        n.attrs.get("variant").and_then(|v| v.as_str()),
        Some("primary")
    );
...
fn combined_real_world_flowchart() {
    ...
    assert_eq!(
        ...
        Some(tautcore_model::NodeShape::Circle)
    );
    assert_eq!(
        node(&out, "check").shape,
        Some(tautcore_model::NodeShape::Diamond)
    );
    assert_eq!(
        node(&out, "db").shape,
        Some(tautcore_model::NodeShape::Cylinder)
    );
    assert_eq!(
        node(&out, "pay").shape,
        Some(tautcore_model::NodeShape::Diamond)
