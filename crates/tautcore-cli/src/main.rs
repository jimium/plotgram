//! Thin CLI: args + file I/O only. Build orchestration lives in `tautcore-compile`.
//!
//! Subcommands (showcase-redesign-2026-08.md §5):
//!   validate <file>          parse + model validate; no layout/render
//!   render   <file> [-o]      .taut → SVG (default: stdout)
//!   measure  <file> [--json]  layout metrics (§5.2): correctness / quality / observation
//!   debug-layout <file> [-o]  .taut → LayoutDebugTrace JSON (debug-inspector.md T1)
//!   explain  <file>           .taut → layout facts (ADR-007): LLM-readable predicates
...
#[derive(Debug, Parser)]
#[command(name = "tautcore", about = "Tautcore DSL → SVG / draw.io")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn run_explain(input: &PathBuf) -> ExitCode {
    ...
    match tautcore_compile::build_explain(&source, &BuildOptions::default()) {
        Ok(text) => {
            println!("{text}");
            ExitCode::SUCCESS
        }
...
fn run_debug_layout(input: &PathBuf, output: Option<PathBuf>) -> ExitCode {
    ...
    match tautcore_compile::build_debug_trace(&source, &BuildOptions::default()) {
        Ok(trace) => {
...
// tautcore-compile::audit (pure, WASM-safe).
...
fn run_measure(input: &PathBuf, json: bool) -> ExitCode {
    ...
    let report = match built {
        ...
        Err(e) => {
            ...
            let status = match &e {
                tautcore_compile::BuildError::Parse(_) => "parse-error",
                _ => "render-error",
            };
            json!({
                "schema_version": METRICS_SCHEMA_VERSION,
    ...
    // `tautcore measure` call surfaces defects; snapshot.sh reads stdout JSON
...
/// dot-split of the filename. Matches the showcase `{layout}/{role}.{slug}.taut`
