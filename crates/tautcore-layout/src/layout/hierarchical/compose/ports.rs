...
//!
//! Author tiers on edges (tautcore_model::port::PortConstraint):
//!
...
use tautcore_algo::orientation::{Orientation as AlgoOrientation, Side};
use tautcore_engine_api::LayoutError;
use tautcore_model::port::{AlongSpec, PortConstraint, Side as ModelSide};
use tautcore_model::{policy_for, ShapePortPolicy};
...
mod tests {
    ...
    use tautcore_model::port::Side as ModelSide;
    ...
    fn reversed_edge_free_ports_prefer_cross_axis_side() {
        ...
        let graph = RealGraph {
            ...
            shapes: vec![tautcore_model::NodeShape::DEFAULT; 2],
            edges: vec![RealEdge {
                edge_id: "back".into(),
                original_source: 0, // a
                original_target: 1, // b
    ...
    fn person_shape_rejects_free_north() {
        ...
        graph.shapes[1] = tautcore_model::NodeShape::Person; // b
        graph.shapes[2] = tautcore_model::NodeShape::Person; // c
        let ports = assign_ports(&graph, &plan, AlgoOrientation::Tb, false)
            .unwrap()
            .ports;
    ...
    fn diamond_capacity_soft_overflows_same_face_primary() {
        let (mut graph, plan) = small_plan_and_graph();
        graph.shapes[0] = tautcore_model::NodeShape::Diamond; // a: two FREE South outs
        let ports = assign_ports(&graph, &plan, AlgoOrientation::Tb, false)
            .unwrap()
            .ports;
    ...
    fn fixed_side_north_on_person_wins() {
        let (mut graph, plan) = small_plan_and_graph();
        graph.shapes[1] = tautcore_model::NodeShape::Person;
