//! P1 cycle removal: `tautcore_algo::fas::greedy_fas` wrapper.
//!
...
/// Deterministic: `tautcore_algo::fas::greedy_fas` breaks ties by
...
pub fn remove_cycles(graph: &mut RealGraph) {
    ...
    let reversed = tautcore_algo::fas::greedy_fas(graph.ids.len(), &pairs);
...
mod tests {
    ...
    fn graph(ids: &[&str], edges: &[(usize, usize)]) -> RealGraph {
        ...
        RealGraph {
            ids,
            index_of,
            shapes: vec![tautcore_model::NodeShape::DEFAULT; group_path.len()],
            group_path,
            edges,
            self_loops: Vec::new(),
            ..Default::default()
