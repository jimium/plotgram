...
//! the physical orientation (`orient.rs`, [`tautcore_algo::orientation`]).
...
use tautcore_algo::orientation::{self as algo_orient, Orientation as AlgoOrientation};
use tautcore_engine_api::{
    EdgeGeometryMode, LayoutAlgorithm, LayoutError, LayoutInput, LayoutOutput, LayoutWarning,
};
use tautcore_model::diagnostics::{
...
use tautcore_model::result::{EdgePath, EdgePlacement, GroupPlacement, NodePlacement};
    ...
    for ce in &canonical_edges {
        if let Some(bends) = ink::verify::polyline_bend_count(&ce.path) {
            if bends > params.max_bends_budget as usize {
                diagnostics
                    .relaxations
                    .push(tautcore_model::diagnostics::Relaxation {
...
fn canonical_sizes(
    real_graph: &model::RealGraph,
    node_sizes: &tautcore_model::sizes::NodeSizes,
    orientation: algo_orient::Orientation,
...
fn collect_labeled_groups(
    groups: &[tautcore_model::graph::Group],
) -> std::collections::BTreeSet<String> {
    let mut out = std::collections::BTreeSet::new();
    fn walk(groups: &[tautcore_model::graph::Group], out: &mut std::collections::BTreeSet<String>) {
        for g in groups {
            if g.label.is_some() {
...
mod tests {
    ...
    fn empty_partition_grid_is_bit_identical_to_no_partition() {
        ...
        let paths = |o: &LayoutOutput| -> Vec<(String, tautcore_model::result::EdgePath)> {
            o.edges
                .iter()
