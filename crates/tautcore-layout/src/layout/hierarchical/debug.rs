...
use tautcore_model::diagnostics::{PartitionBandObs, PartitionRowBandObs};
...
fn project_ports(
    cap: &Captures<'_>,
    algo_orient: tautcore_algo::orientation::Orientation,
        ...
        for (end, node_idx, resolved, constraint) in [
            ...
            out.push(PortDebug {
                ...
                slot: match resolved.along {
                    tautcore_model::port::AlongSpec::Ordered { order, .. } => Some(order),
                    _ => None,
                },
...
fn constraint_str(constraint: Option<&tautcore_model::port::PortConstraint>) -> &'static str {
    use tautcore_model::port::PortConstraint::*;
    match constraint {
        None => "free",
...
fn side_str(side: tautcore_model::port::Side) -> String {
