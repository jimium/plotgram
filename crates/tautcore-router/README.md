...
独立边路由 crate。与 `tautcore-engine`（布局 facade）物理隔离，只依赖底层 API 和算法库。

## 依赖关系

```text
tautcore-model          (几何原语: Point, Rect, Side)
     ^
tautcore-engine-api     (RouteScene, EdgeRouter trait, LayoutError)
     ^
tautcore-algo           (VPSC, crossing, path_ortho)
     ^
tautcore-router         ← 本 crate
     ^
tautcore-engine         (facade; 消费 router)
```

**纪律**：本 crate 永不依赖 `tautcore-engine` 或任何 layout 实现。
...
```text
改算法 → cargo test -p tautcore-router（门禁绿？）
