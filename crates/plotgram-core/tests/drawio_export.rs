...
use plotgram_core::render::encode::drawio::{
    encode_scene_inner, DrawioExportOptions, DrawioRenderer, ExportReport,
};
use plotgram_core::render::encode::FormatEncoder;
use plotgram_core::render::scene::export_scene;
...
fn create_flowchart_prepared() -> PreparedDiagram {
    ...
    diagram.attributes.push(DiagramAttribute {
        key: "title".to_string(),
        value: plotgram_core::ast::AttributeValue::String(TextValue::quoted(
...
fn assert_valid_drawio(xml: &str) {
    ...
    assert!(xml.contains("host=\"Plotgram/"), "缺 host: {xml}");
...
"#;
    ...
    // 结构性元数据保留
    assert!(xml.contains("plotgramEntityId="), "应保留 plotgramEntityId: {xml}");
    assert!(
        xml.contains("plotgramRelationIndex="),
        "应保留 plotgramRelationIndex: {xml}"
    );
    // 形状保留
    assert!(
...
fn drawio_encoder_produces_valid_mxfile() {
    ...
    assert!(xml.contains("host=\"Plotgram/"));
