...
use plotgram_core::layout::compute_layout;
use plotgram_core::prepare::StyleRequest;
use plotgram_core::pipeline::{parse, prepare, render_text};
use plotgram_core::render::{RenderFormat, RenderRequest};
use plotgram_core::validation::validate;
use std::fs;
...
fn collect_pgm_files(dir: &Path, out: &mut Vec<PathBuf>) {
    for entry in fs::read_dir(dir).expect("read showcase dir") {
        let entry = entry.expect("read entry");
        let path = entry.path();
        if path.is_dir() {
            collect_pgm_files(&path, out);
        } else if path.extension().is_some_and(|e| e == "pgm") {
            out.push(path);
        }
    }
}
...
fn showcase_parse_prepare_validate_render() {
    ...
    collect_pgm_files(&root, &mut files);
    files.sort();
    assert!(!files.is_empty(), "expected at least one showcase .pgm file");

    let mut failures = Vec::new();

    for path in &files {
        let rel = path.strip_prefix(&root).unwrap().display();
        let source = fs::read_to_string(path).expect("read pgm");
...
fn showcase_architecture_group_containment() {
    ...
    let mut files = Vec::new();
    collect_pgm_files(&arch_dir, &mut files);
    files.sort();
    assert!(
        !files.is_empty(),
        "expected at least one architecture showcase .pgm file"
    );
    ...
    for path in &files {
        ...
        for v in &violations {
            let dir = match v.kind {
                plotgram_core::layout::ContainmentViolationKind::TopOverflow => "top",
                plotgram_core::layout::ContainmentViolationKind::BottomOverflow => "bottom",
                plotgram_core::layout::ContainmentViolationKind::LeftOverflow => "left",
                plotgram_core::layout::ContainmentViolationKind::RightOverflow => "right",
            };
            failures.push(format!(
