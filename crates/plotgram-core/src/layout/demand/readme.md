...
- [`docs/architecture/方案计划/路由压力模型收缩与复用方案-2026-07.md`](../../../../../docs/architecture/方案计划/路由压力模型收缩与复用方案-2026-07.md)
- [`docs/architecture/方案计划/压力模型算法吸收计划-P1-P4-2026-07.md`](../../../../../docs/architecture/方案计划/压力模型算法吸收计划-P1-P4-2026-07.md)
...
- **能力**：  
  ...
  - 路由打分可对「走在 OVER 廊上」的路径 soft 罚（`PLOTGRAM_CORRIDOR_SOFT=0` 可关）。
...
- **能力**：画布热点视图；进 score 的 ζ 项；`PLOTGRAM_DUMP_GRID_DEMAND=1` 可 dump。  
...
| 能力 | 怎么用 | 改几何吗 |
|------|--------|----------|
| 路由前压力快照 | `PressureSnapshot::compute` / `PLOTGRAM_DUMP_PRE_ROUTE_PRESSURE` | 否 |
| 边难度 dump | `PLOTGRAM_DUMP_EDGE_DIFFICULTY` | 否 |
| 网格 dump | `PLOTGRAM_DUMP_GRID_DEMAND` | 否 |
...
| 路由边序 | `layer_order` 读难度分（同 rank 高分略提前；`PLOTGRAM_EDGE_ORDER_SCORE=0` 可关） | 否（只改路由顺序） |
| 端口侧 | 默认：超载侧拒合并 + **Compact 组内 pitch 温和加大**（保留 Concentrate；`PLOTGRAM_PORT_PRESSURE_SLOT=0` 可关） | 否 |
| 路由 soft | `OrthoRoutingContext::with_corridor_demands` + scoring | 否（只影响选哪条候选） |

开关：`PLOTGRAM_PRESSURE_BUDGET=0` 关全部加缝；`PLOTGRAM_EDGE_PRESSURE_BUDGET=0` 仅关边级 hits/grid；`PLOTGRAM_EDGE_ORDER_SCORE=0` 关难度分边序；`PLOTGRAM_PORT_PRESSURE_SIDE=0` 关端口超载拒合并；`PLOTGRAM_PORT_PRESSURE_SLOT=0` 关同侧 slot 加大错开；`PLOTGRAM_PORT_PRESSURE_PREFER=1` / `PLOTGRAM_PORT_PRESSURE_RELIEVE=1` 开启更激进的换轴/分流（默认关）。
...
细则与约束见 [`压力模型算法吸收计划-P1-P4-2026-07.md`](../../../../../docs/architecture/方案计划/压力模型算法吸收计划-P1-P4-2026-07.md) §14。**当前不实施。**
