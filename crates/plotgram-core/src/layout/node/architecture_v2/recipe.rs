//! Architecture 布局配方（Recipe）。
//!
//! Phase 10: 显式编排 architecture 布局的完整生命周期。
//! 复用 kernel solver，但不复用 flowchart 语义（split/end/pendant）。
//!
//! ## 编排流程
//!
//! ```text
...
//! ```
//!
//! ## 与 FlowchartRecipe 的区别
//!
//! - 保留 macro/intra 两阶段语义
//! - 不使用 flowchart 的 split/end/pendant 结构目标
//! - 组内 layered path 委托 LayeredKernel
...
/// Architecture 布局配方。
///
/// 编排 architecture 布局的完整生命周期：compile → solve → product → freeze。
/// 复用 kernel solver，但不复用 flowchart 语义。
pub struct ArchitectureRecipe;

impl ArchitectureRecipe {
    /// 执行完整的布局流程。
    ///
    ...
    pub fn execute(problem: &CoordinateProblem) -> RecipeOutput {
        ...
        if !audit.passed() {
            crate::perf_log!(
                "[arch-recipe] P0 audit FAILED: {} violations, max={:.2}px",
