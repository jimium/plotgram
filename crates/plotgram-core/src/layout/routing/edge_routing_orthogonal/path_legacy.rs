//! Legacy template-enumeration path selection.

//! Path building for orthogonal edge routing

use super::*;
use super::path::{port_aware_elbow, port_outward, source_group_exit_stub_len};
...
///
/// Phase 2：`RouteKernelKind::LexAStar`（默认）走 kernel ResourceGraph + 词典序 A*；
/// `Legacy` 保留模板枚举（调试回退）。
pub(super) fn select_best_path_legacy(
    ctx: &OrthoRoutingContext,
    pair: &EndpointPair,
    scorer: &dyn CandidateScorer,
    ...
    if path_kernel::use_lex_astar(ctx.cfg) {
        return path_kernel::select_best_path_lex_astar(ctx, pair, stats);
    }
