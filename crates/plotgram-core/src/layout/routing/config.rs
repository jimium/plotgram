...
pub struct RoutingConfig {
    ...
    /// 端口求解器 v2 反向 stub 惩罚（原 `PLOTGRAM_PORT_SOLVER_V2`，默认 true）。
    pub port_solver_v2: bool,
    /// 同侧 slot 压力错开（原 `PLOTGRAM_PORT_PRESSURE_SLOT`，默认 true）。
    pub port_pressure_slot: bool,
    /// 廊级 OVER soft 惩罚（原 `PLOTGRAM_CORRIDOR_SOFT`，默认 true）。
    pub corridor_soft: bool,
    /// 远离惩罚每 px 费率（原 `PLOTGRAM_AWAY_PENALTY`，默认 2.0）。
    pub away_penalty_rate: f64,
    /// stub 出组惩罚量（原 `PLOTGRAM_STUB_EXIT_PENALTY`，默认 120.0）。
