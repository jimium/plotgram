...
use crate::error::{DiagnosticError, PlotgramError, Result};
...
/// 解析 Plotgram 源文本为 Diagram AST
///
/// 始终走 fallback 路径以收集尽可能多的错误（spec §6.1）。
/// 如果存在任何错误，返回 `Err(PlotgramError::Parse(all_errors))`。
pub fn parse(source: &str) -> Result<Diagram> {
    let (diagram, errors, _warnings) = parse_with_diagnostics(source);
    if errors.is_empty() {
        diagram.ok_or_else(|| PlotgramError::parse_msg("no diagram produced"))
    } else {
        Err(PlotgramError::Parse(errors))
    }
}
...
impl Parser {
    ...
    fn parse_file(&mut self) -> Result<Diagram> {
        ...
        if self.at_eof() {
            ...
            return Err(PlotgramError::parse_msg("missing diagram declaration"));
        }
        ...
        if !matches!(self.peek_kind(), TokenKind::Diagram) {
            ...
            return Err(PlotgramError::parse_msg("expected 'diagram'"));
        }
        self.advance();
        ...
        if !self.expect_lbrace() {
            return Err(PlotgramError::parse_msg("expected '{'"));
        }
        ...
        if !self.errors.is_empty() {
            return Err(PlotgramError::Parse(self.errors.clone()));
        }
    ...
    fn parse_diagram_type(&mut self) -> Result<DiagramType> {
        ...
        let dt = match self.peek_kind() {
            ...
            _ => {
                ...
                return Err(PlotgramError::parse_msg("invalid diagram type"));
            }
        };
        self.advance();
        Ok(dt)
