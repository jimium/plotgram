...
pub fn svg_to_pixmap(
    ...
    let tree = usvg::Tree::from_str(svg_data, &opts)
        .map_err(|e| PlotgramError::render_internal_msg(format!("failed to parse svg for {format_name} encoding: {e}")))?;

    let width = tree.size().width() as u32;
    let height = tree.size().height() as u32;
    if width == 0 || height == 0 {
        return Err(PlotgramError::render_internal_msg(format!(
            "{format_name} svg dimensions are zero"
        )));
    }

    let mut pixmap = resvg::tiny_skia::Pixmap::new(width, height)
        .ok_or_else(|| PlotgramError::render_internal_msg(format!("failed to allocate {format_name} pixmap")))?;
    ...
    .ok_or_else(|| {
        PlotgramError::render_internal_msg(format!("failed to build {format_name} image buffer"))
    })?;
...
pub fn render_svg_to_image_bytes(
    ...
    img.write_to(&mut cursor, format)
        .map_err(|e| PlotgramError::render_internal_msg(format!("failed to encode {format_name}: {e}")))?;
...
pub fn render_svg_to_image_file(svg_data: &str, output_path: &str) -> Result<()> {
    let opts = build_usvg_options();
    let tree = usvg::Tree::from_str(svg_data, &opts)
        .map_err(|e| PlotgramError::render_internal_msg(format!("failed to parse svg for rasterization: {e}")))?;

    let width = tree.size().width() as u32;
    let height = tree.size().height() as u32;
    if width == 0 || height == 0 {
        return Err(PlotgramError::render_internal_msg("svg dimensions are zero"));
    }

    let mut pixmap = resvg::tiny_skia::Pixmap::new(width, height)
        .ok_or_else(|| PlotgramError::render_internal_msg("failed to allocate pixmap"))?;
    ...
    .ok_or_else(|| PlotgramError::render_internal_msg("failed to build image buffer"))?;

    img.save(output_path)
        .map_err(|e| PlotgramError::render_internal_msg(format!("failed to save file: {e}")))?;
