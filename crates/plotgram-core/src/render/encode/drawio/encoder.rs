...
impl<'a, 's> DrawioEncoder<'a, 's> {
    ...
    fn check_unsupported(&mut self) -> Result<()> {
        ...
        if !self.options.allow_unsupported_diagram_types {
            return Err(crate::error::PlotgramError::render_internal_msg(format!(
        ...
        match self.options.fallback {
            DrawioFallback::Error => Err(crate::error::PlotgramError::render_internal_msg(
...
<mxfile host="Plotgram/{crate_version}" agent="Plotgram/{crate_version}" version="1.0">
  <diagram id="plotgram-diagram" name="{title}">
    ...
    fn write_groups(&mut self) {
        ...
        for (gid, group, abs_x, abs_y, has_label, parent_gid) in group_entries {
            ...
            if self.options.include_export_metadata {
                style_parts.push(format!("plotgramGroupId={}", sanitize_id(&gid)));
            }
...
"#,
    ...
    fn write_edge(&mut self, edge: &ExportEdge<'_>) {
        ...
        // 元数据
        if self.options.include_export_metadata {
            style_parts.push(format!("plotgramRelationIndex={}", edge.index));
            style_parts.push(format!("plotgramDegrade={}", routing.tier.as_str()));
        }
        let style = style_parts.join(";");
...
"#,
    ...
    fn write_node(&mut self, node: &ExportNode<'_>) {
        ...
        if self.options.include_export_metadata {
            style_parts.push(format!("plotgramEntityId={}", entity_id));
            if tier > DegradeTier::L0 {
                style_parts.push(format!("plotgramDegrade={}", tier.as_str()));
            }
        }
        let style = style_parts.join(";");
