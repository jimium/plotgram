...
- UTF-8 XML，根元素 `<mxfile host="Plotgram/{version}">`
...
- `plotgramEntityId` — 节点
- `plotgramRelationIndex` — 边
- `plotgramGroupId` — 分组
- `plotgramDegrade` — 该元素降级级别
...
| `include_export_metadata` | `true` | plotgram* 元数据 |
...
集成测试：`crates/plotgram-core/tests/drawio_export.rs`（全管线 + XML 结构、颜色、箭头、端口等断言）。
