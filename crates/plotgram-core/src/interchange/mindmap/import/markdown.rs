...
fn parse_atx_outline(
    ...
    while i < lines.len() {
        match &lines[i].kind {
            ...
            LineKind::HtmlComment(content) => {
                // 解析 <!-- plotgram:entity-id=xxx -->
                if let Some(entity_id) = parse_plotgram_entity_id(content) {
...
/// 解析 <!-- plotgram:entity-id=xxx --> 注释。
fn parse_plotgram_entity_id(content: &str) -> Option<String> {
    let content = content.trim();
    if let Some(rest) = content.strip_prefix("plotgram:entity-id=") {
