...
2. `cargo test -p tautcore-algo` 单独绿；  
3. 再在 `tautcore-engine` 的 hierarchical / route 里接线（可先只接一件）。
...
| 允许 | 禁止 |
|------|------|
| `f64`、`usize` 索引、本 crate 内小型 `Point`/`Rect` 或日后薄依赖 `tautcore-model::geometry` | 依赖 `tautcore-engine` / `engine-api` 的 run 类型 |
| `thiserror` | `if diagram_type` / `profile` |
| 稳定序：`BTreeMap` / 显式 sort | `HashMap` 迭代驱动结果 |
| 迭代算法显式上限 + 超限错误 | 静默死循环 |

当前 **不**依赖 `tautcore-model`，减少编译耦合；若 `orientation` 需要与 model 的 `Side` 对齐，再增加 **仅 geometry/port 枚举** 的薄依赖，并在本文件注明。
...
- [ ] 六件均有可调用 API + 表驱动测试  
- [ ] `cargo test -p tautcore-algo` 稳定绿、确定性双跑（关键用例）  
