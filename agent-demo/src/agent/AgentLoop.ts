 ...
 * maxIterations 默认 6（演示场景，防死循环消耗额度）
 */
...
export async function runAgentLoop(
  ...
  for (let iteration = 0; iteration < config.maxIterations; iteration++) {
    ...
    // 3. 一次性 push 包含全部 tool_calls 的 assistant 消息(转成 OpenAI 兼容格式)
    messages.push({
      role: 'assistant',
      content: response.content || '',
...
function updateStateFromResult(toolName: string, result: unknown): StateUpdate {
  ...
  if (toolName === 'render') {
    const r = result as { success: boolean; text?: string | null };
    if (r.success && r.text) {
      update.svg = r.text;
    } else if (!r.success) {
  ...
  if (toolName === 'apply_patch') {
    const r = result as { success: boolean; source?: string | null };
