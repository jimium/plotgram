...
**路径**：`crates/drawify-core/src/layout/edge/edge_routing_orthogonal/lane_assignment.rs`
...
**路径**：`crates/drawify-core/src/layout/edge/edge_routing_orthogonal/channel_load.rs`
...
### 编译
```bash
cargo build --release -p drawify-core
```

### 单元测试
```bash
cargo test --release -p drawify-core
```

### SVG 验证
生成 `showcase/architecture/c.layout-stress-nested.dfy` 的 SVG：
