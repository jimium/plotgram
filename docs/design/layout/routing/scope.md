...
| 模块 | 职责 |
|------|------|
| `plotgram-engine-api` | `EdgeRouter` / `RouteInput`（过渡）/ 目标 `RouteScene` |
| `plotgram-engine/src/route/core` | 无策略原语 |
| `plotgram-engine/src/route/orthogonal` | 本 Router 实现（可后拆 crate） |
