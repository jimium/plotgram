...
Sugiyama 骨架 + Atlas 三相写权（组合 → 度量 → Ink）。详见 [pipeline.md](pipeline.md)。
...
- **默认**：内建正交 Ink（`EdgeGeometryMode::Builtin`），组合相决定端口/通道拓扑。  
...
| 文档 | 用途 |
|------|------|
| [from-yfiles-reference.md](from-yfiles-reference.md) | **yFiles 参考文库启发**：Hier 构造与 v1 Atlas 迁移（慢慢读） |
