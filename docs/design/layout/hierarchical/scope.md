...
| **PartitionGrid** | 正交列/行（ADR-008）；model/DSL 已定；组合/度量 **已消费**（PG-0–PG-4）→ [phases/partition-grid.md](phases/partition-grid.md)；render 只读 band 画底色/标题 |
...
3. **PartitionGrid 引擎消费** — 已落地（PG-0–PG-4：列连续块 + 行区间 + 四向 + Weak/Strong 共址）；render 只读 band 画泳道底色/标题。切片方案：[phases/partition-grid.md](phases/partition-grid.md)。  
