...
4. 平局按固定 Side 顺序；
5. 同侧边按**邻层邻接元** `(layer_order, rank, EdgeId)` 排序（含 long-edge dummy）。远真实端的 `layer_order` 跨层不可比，会把回边槽倒到最左，P4 再把廊锚在错槽上。邻层 dummy 与短边叶左右反了，是 P3 整链 sift 的事，不在 P7 用远真实端撒谎。
