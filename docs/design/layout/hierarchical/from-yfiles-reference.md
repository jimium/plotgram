...
> 相关：[pipeline.md](pipeline.md) · [scope.md](scope.md) · [写权纪律](../write-authority.md) · [reference 13 选型与路线图](../../../reference/yfiles/13-实现路线图与选型.md)
...
| **08** | Weak/StrongMacro、泳道应收敛到「边界 dummy + 连续块 + VPSC 组框」，不是两套组几何 |
...
- 自由度用 `LayerWriter` / `CoordWriter` / `RouteWriter` 等收紧。  
...
- 组与泳道都是「层内连续块」——**边界 dummy + 受约束排序**；  
- 组框是 VPSC（或等价分隔约束）变量，**禁止事后包围盒当真源**。  

对照 archive 30·R3：Weak / StrongMacro 应是收缩 / 连续块**参数**；Horizontal 堆叠 ≠ PartitionGrid。  
...
- 真 PartitionGrid DSL（泳道观感先用 group + 连续块）  
- organic / 力导向  
