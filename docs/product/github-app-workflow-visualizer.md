...
做一个免费的 GitHub App：用户在 PR 或 Issue 评论中 `@plotgram-bot`，即触发对 `.github/workflows/*.yml` 的解析，用 **Plotgram 渲染 SVG**，将图片链接回贴到评论线程。

- **触发方式**：用户主动 `@`，不做每次 push 自动刷屏
- **输出格式**：仅 Plotgram SVG，不使用 Mermaid（竞品，且无法展示 Plotgram 渲染能力）
- **存储方案**：Cloudflare R2 短生命周期缓存 + 内容去重，目标月成本 $0–10
- **品牌目标**：PR 评论与 SVG 页面带 Plotgram 标识，引流至官网 / Playground
...
[success-roadmap.md](success-roadmap.md) 中规划的 GitHub Action 方向是 **`.pgm` → SVG**（用户提交图表，CI 渲染）。本 App 是反向入口：**workflow YAML → Plotgram → SVG**，与 Action 形成同一生态的两侧：

```
用户写 .pgm 架构图  ──Action──►  SVG 附到 PR
...
1. 用户在 GitHub Marketplace 安装 **Plotgram Workflow Visualizer** App
2. 选择要授权的仓库（或整个 org）
...
```text
@plotgram-bot visualize
```

或指定文件：

```text
@plotgram-bot visualize .github/workflows/ci.yml
```
...
![CI Pipeline](https://assets.plotgram.app/svg/a1b2c3....svg)

---
[在 Plotgram 中打开](https://plotgram.app/...) · Powered by [Plotgram](https://plotgram.app)
```
...
后续可在「PR 修改了 workflow 文件」时仅发一行提示，引导用户 `@plotgram-bot visualize`，而不直接贴大图。
...
GitHub issue_comment webhook
  ...
  GitHub API 拉取 workflow YAML
        │
        ▼
  YAML → Plotgram DSL 转换
        │
        ▼
  plotgram-core / plotgram-server 渲染 SVG
        │
...
| 组件 | 用途 |
|------|------|
| `plotgram-core` | 布局与 SVG 渲染 |
| `plotgram-server` | HTTP `/render` 接口，App Worker 可直接调用 |
...
| Workflow 解析器 | YAML → Plotgram AST / DSL |
| R2 存储适配层 | 上传、去重、生命周期 |
...
| 事件 | 用途 |
|------|------|
| `issue_comment` | 监听 `@plotgram-bot` 指令（PR 评论包含在内） |
...
| 公开 URL | `https://assets.plotgram.app/svg/{hash}.svg` |
...
1. **内容去重**：以 `sha256(workflow_yaml + plotgram_version)` 为 key，相同内容只存一份
...
| 方案 | 不采用原因 |
|------|-----------|
| Mermaid 代码块嵌入 PR | 竞品格式，无法展示 Plotgram 能力，与品牌目标冲突 |
...
单次 `@plotgram-bot visualize` 耗时约 1–3 秒：

| 步骤 | 耗时 |
|------|------|
| GitHub API 拉取 YAML | 200ms–1s |
| YAML 解析 | <50ms |
| Plotgram 渲染 SVG | 100ms–500ms |
| 上传 R2 + 回复评论 | 200ms–1s |
...
```bash
# 目标：CLI 跑通 YAML → SVG
plotgram gha visualize .github/workflows/ci.yml -o ci.svg
```
...
- 支持 `@plotgram-bot visualize` 与 `@plotgram-bot visualize <path>`
...
- 公开预览页：`plotgram.app/gh/{owner}/{repo}/workflows/{name}`
- README Badge：`![CI Diagram](https://plotgram.app/badge/{owner}/{repo})`

### Phase 3：差异化

- Workflow 变更 diff 图
- Steps 展开、reusable workflow 架构图
- 与 GitHub Action（`.pgm` → SVG）联动
...
![{workflow_name}](https://assets.plotgram.app/svg/{hash}.svg)

---
[在 Plotgram 中打开](https://plotgram.app/view/{id}) · Powered by [Plotgram](https://plotgram.app)
```
...
|------|------|
| PR 评论 | 每次 `@` 都是一次品牌曝光 |
| SVG 页面 | 点击「在 Plotgram 中打开」引流至 Playground |
| Marketplace | 「Plotgram Workflow Visualizer」搜索入口 |
...
| 用户输入 | 行为 |
|----------|------|
| `@plotgram-bot visualize` | 渲染当前上下文中的默认 workflow（或 PR 中变更的 workflow） |
| `@plotgram-bot visualize ci.yml` | 渲染指定路径 |
| `@plotgram-bot help` | 回复帮助与支持的命令列表 |

`issue_comment` webhook 收到后：

1. 检查 `comment.body` 是否包含 `@plotgram-bot` 或 `@plotgram[bot]`
...
| 竞品已有 YAML 可视化工具 | 差异化在 Plotgram 渲染质量、diff 能力、生态联动 |
