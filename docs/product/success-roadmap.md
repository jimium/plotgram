# Plotgram 成功路线图：必须做对的五件事

> 基于竞品分析与项目现状，识别出决定 Plotgram 成败的五个关键领域。

## 0. 现状概览

Plotgram 是一个为 AI Agent 设计的图表 DSL 语言和渲染引擎，核心差异化：
...
## 1. 让 LLM 真的能写好 Plotgram

**为什么关键**："为 AI Agent 设计"是核心卖点，但目前只是 claim，没有数据支撑。如果 LLM 生成 Plotgram 的正确率不比 Mermaid 高，卖点就不成立。

### 1.1 LLM 语法正确率基准测试

- 用 GPT-4 / Claude / Gemini 对比生成 Mermaid vs Plotgram
- 测试维度：语法正确率、语义正确率、首次生成可用率、修复轮次
- 场景覆盖：6 种图表类型 × 简单/中等/复杂
- 目标：Plotgram 首次正确率 > Mermaid 20%+

### 1.2 LLM Prompt Template / System Prompt

- 提供开箱即用的 system prompt，让任何 LLM 都能高质量生成 Plotgram DSL
...
- 部署到 plotgram.dev 或类似域名
- 打开即用，零安装
- WASM 驱动，全部在浏览器运行

### 2.2 "AI 生成"按钮

- 用户输入自然语言描述 → 调用 LLM → 生成 Plotgram DSL → 实时渲染
- 这是 Eraser.io 的核心体验，Plotgram 必须做到
...
- 示例要展示 Plotgram 独有能力：语义实体、图标库、笔触皮肤

### 2.4 笔触皮肤一键切换

- Excalidraw / Blueprint / Neon Glow 等皮肤是 Plotgram 独有的杀手级特性
- Mermaid 和 D2 都没有
- 必须在首页就能看到切换效果
...
**为什么关键**：Mermaid 统治这个领域的核心原因是 GitHub 原生渲染。Plotgram 无法复制这一点，但可以找到侧翼入口。

### 3.1 GitHub Action（P1）

- 提交 `.pgm` 文件 → CI 自动渲染 SVG/PNG → 附加到 PR comment
- 这是最低成本的生态切入点
- 用户不需要 GitHub 支持 Plotgram，只需要一个 Action

### 3.2 VS Code 插件（P1）

- 语法高亮 + 实时预览
- Mermaid 和 D2 都有，Plotgram 也必须有
- 基于 WASM，可以在插件内直接渲染，无需外部服务

### 3.3 Kroki 集成（P2）

- Kroki 是通用图表渲染 API，支持 20+ 语言
- 让 Kroki 支持 Plotgram 格式，就能进入所有使用 Kroki 的平台
- 需要向 Kroki 提交 PR 或提供独立渲染服务

### 3.4 Markdown 代码块渲染（P2）

- 至少支持 ````plotgram` 在常见静态站点生成器中渲染
...
**为什么关键**：AST 级别的 diff/patch 是 Plotgram 独有的能力，但目前看不到真实使用场景。如果场景跑不通，就是过度设计。

### 4.1 "图表即代码审查"场景

- PR 中 `.pgm` 文件的 diff 不只是文本 diff，而是语义 diff
...
**为什么关键**：Mermaid 有 70k+ GitHub stars，D2 有 18k+。Plotgram 要被看见。
...
2. **"Plotgram vs Mermaid vs D2：LLM 生成正确率对比"** — 核心差异化证明
...
- V2EX / 掘金 / 知乎：中文开发者社区
- Twitter/X：@plotgram 官方账号

### 5.4 AI 编程工具集成

- **MCP Server**：让 Cursor / Claude Code / Copilot 等 AI 编程工具的 agent 能直接生成 Plotgram DSL
- 这是最直接的方式——AI 工具是 Plotgram 的天然分发渠道
- 提供 `plotgram-mcp` crate，暴露 render/validate/diff 能力
...
| 项目 | 定位 | GitHub Stars | Plotgram 的差异化 |
|------|------|-------------|-----------------|
...
**关键洞察**：市场上没有其他项目同时做到"为 AI 生成优化 + AST 级别操作 + 笔触皮肤 + 自动布局"这个组合。Plotgram 的窗口期在于**在 Mermaid/D2 意识到 AI-native 需求之前**建立先发优势。
