# D2 vs Plotgram 深度源码对比与借鉴报告

> 基于 D2 (terrastruct/d2) 本地源码的深入分析，聚焦 Plotgram 可借鉴的设计模式与实现方案。
...
- [十、Plotgram 已有优势](#十plotgram-已有优势)
- [总结：借鉴路线图](#总结借鉴路线图)
...
### Plotgram：三阶段管线（Lexer → Parser → Expand）
...
Plotgram 的 prepare 阶段目前只做两件事：补全 `entity.type` 默认值 + 物化样式到 `attributes.style`。

### 借鉴建议：引入 IR 层，解耦语义转换

**D2 的优势**：所有"语法糖"处理都集中在 IR 阶段，compiler 只需处理纯粹的语义映射。这让每个阶段的职责极其清晰。

**Plotgram 的风险**：如果未来要支持变量、import、class 继承等特性，直接在 Parser 或 Expand 中堆砌会导致 `prepare()` 函数膨胀。
...
### Plotgram：静态 Trait 注册

Plotgram 的 `layout/mod.rs` 使用 Rust trait：
...
**D2 的亮点**：`PluginFeature` + `FeatureSupportCheck` 是一个极好的设计——它让布局引擎声明自己支持什么，系统在运行时检查用户是否使用了不支持的特性。这比 Plotgram 的 `applicable_diagram_types()` 更精细。
...
Plotgram 目前对整个 Diagram 执行一次布局，没有嵌套子图的概念。
...
**Steps** 对 Plotgram 的 AI Agent 场景最有价值——Agent 可以生成一系列步骤图来演示流程演变。
...
### Plotgram：Theme + GraphicStyle 双轨

Plotgram 的 `graphic_style/mod.rs` 分离了颜色（Theme）和笔触（GraphicStyle），这是 D2 没有的设计。Plotgram 的 `StyleMap` 还携带了 `StyleSource` 溯源信息，这也是 D2 没有的。
...
1. **补全**：基于 Plotgram 的 keywords 定义，实现 `get_keyword_context` + `get_completions`
2. **Oracle**：基于 Plotgram 已有的 `diff/patch` 模块，实现 `create_entity`、`set_attribute`、`delete_entity` 等程序化编辑 API

Plotgram 的 `DiagnosticError` 已经有结构化的 `Suggestion` + `FixAction`，这比 D2 的错误信息更适合 LSP 的 `codeAction`。
...
### Plotgram：PreparedDiagram → Renderer

Plotgram 的渲染器直接消费 `PreparedDiagram`，没有独立的 Export 层。

### 借鉴建议：引入 Export 层

**D2 的优势**：Export 层将布局结果转为渲染器无关的纯数据结构，让多个渲染器（SVG、PNG、ASCII）共享同一份导出逻辑。

**建议**：Plotgram 目前的 SVG 渲染器直接读取 `PreparedDiagram`，如果未来要支持 PNG/ASCII 等格式，应引入 Export 层将布局结果标准化。
...
### Plotgram：Excalidraw GraphicStyle

Plotgram 的 `excalidraw` 模块实现了类似的手绘效果，但通过 Rust 原生实现而非 JS 引擎。

**对比**：Plotgram 的原生实现性能更好，但 D2 的 rough.js 生态更成熟（支持更多形状变体）。

---

## 十、Plotgram 已有优势

以下是 Plotgram 已经具备、但 D2 不具备的设计优势：
...
Plotgram 的 `diff/compare.rs` 支持语义级图表差异比较，D2 没有这个能力。

### 4. GraphicStyle 双轨

Theme（颜色）+ GraphicStyle（笔触）分离，比 D2 的 Theme + Sketch 开关更灵活。Plotgram 支持 7 种笔触风格（Standard、Excalidraw、Blueprint、NeonGlow、SpatialClarity、Stipple、CrossHatch），D2 只有 Sketch on/off。
...
|--------|--------|----------|----------|----------|
| **P0** | DSL 自动格式化 (`plotgram fmt`) | d2format | AI 生成代码规范化 | 中 |
...
Plotgram 在 AI Agent 友好性方面（结构化诊断、AST diff/patch、StyleSource 溯源、降级解析）已经领先 D2。最值得借鉴的是 D2 的**工程化成熟度**——自动格式化、Feature 声明检查、Board 系统、Oracle API 这些让工具从"能用"变成"好用"的关键基础设施。
