...
`plotgram` 是 Plotgram 的命令行入口，封装 parse → prepare → validate → layout → render 管线。

> 实现：`crates/plotgram-cli/`
...
```bash
cargo build -p plotgram-cli
cargo install --path crates/plotgram-cli   # 可选

plotgram --help
plotgram render --help
```
...
```bash
plotgram render diagram.pgm
plotgram render diagram.pgm -f png -o out.png
plotgram render outline.md --input-format md-outline -f svg
```
...
| `--fonts-dir` | 字体目录（PNG/WebP 中文渲染）；也可用环境变量 `PLOTGRAM_FONTS_DIR` |
...
plotgram validate diagram.pgm --format json
...
```bash
plotgram export diagram.pgm > ast.json
```
...
比较两份 `.pgm` 在 **RawDiagram** 层的语义差异（diff2，不含 theme 物化差异）。
...
```bash
plotgram patch diagram.pgm changes.json -o patched.json
plotgram patch diagram.pgm changes.json   # stdout
```
...
```bash
plotgram diff -o base.pgm -n edited.pgm --format json > delta.json
# Agent 编辑 delta.json 或生成新 patch
plotgram patch base.pgm delta.json -o result.json
```
