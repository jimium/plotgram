...
`plotgram-eval` 对布局与边路由算法做**量化评分、A/B 对比和回归检测**。与 LayoutLint 互补：eval 回答「好不好」，lint 回答「哪里错了」。

> 实现：`crates/plotgram-eval/`  
> Crate 内详细 API 表：[crates/plotgram-eval/readme.md](../../crates/plotgram-eval/readme.md)

**注意**：当前 **没有独立的 `plotgram-eval` CLI 二进制**，需通过 **Rust API** 或自建脚本调用。下文示例均为库用法。
...
```rust
use plotgram_core::pipeline::parse;
use plotgram_eval::engine::{EvalEngine, presets};
...
```rust
use plotgram_eval::history::HistoryStore;
...
```rust
use plotgram_eval::report::EvalReport;
...
```bash
cargo test -p plotgram-eval -- --nocapture   # 跑 crate 内测试
# 或编写 bins/eval-cli.rs 调用 EvalEngine
```

若需要一等 CLI，可在 `plotgram-eval/Cargo.toml` 增加 `[[bin]]` 并封装 readme 中的命令表。
