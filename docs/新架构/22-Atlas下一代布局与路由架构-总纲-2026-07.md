...
> 日期：2026-07-26  
> 状态：**设计主文档**（对应门禁豁免 [`AGENTS.md`](../../AGENTS.md) §10）  
...
| **闸口** | `Gate` | 组边界上边的合法进出点。跨组边在相 I 就选定 gate 序列，不再事后检测穿组 |
...
- 每个组框的每条边（上/下/左/右）离散化为若干 `Gate` 槽位，容量 = 边长 / 最小间距
...
- **穿组从「检测项」变成「不可表达」**：没有 gate 就没有跨越，`H2GroupInterior` 由构造保证
...
顶点：
  ...
  TrackSlot(track_id, lane_index)     # 层间缝 / 层内走廊 / 组间走廊
  Gate(group, side, slot)             # 相 I.4 已定
边：
  相邻 track 之间、track 与 port/gate 之间的合法转移
容量：
  track 的 lane 数（可增长——lane 数是 Demand，不是上限）
```

规模对比：OVG 顶点数 ~ O(节点数²)（代码里 `ovg_enabled` 限节点 ≤50）；通道图顶点数 ~ O(层数 × 每层通道数)，**通常小两个数量级**，可以对全部边做完整搜索而非贪心 + rip-up。

- **单边选路**：`lex_astar`（已存在）在通道图上跑，`LexCost` 语义不变
- **全局协调**：边按难度序（复用 `demand::score_edges`）依次选路；冲突时**词典序有界回溯**（泛化今天的 rip-up/reroute，但对象是 track 占用而非像素）
- **合流（trunk merge）**：同 track 同向且共享后缀的边自动成束——今天 `semantic_trunk_merge` 要在几何上找共线段，Atlas 里「共享 track 序列后缀」就是合流的定义，**是拓扑事实不是几何巧合**

**关键收益**：track 的 lane 数是**输出**不是输入。5 条边挤一条 track，track 就声明 `Demand{min: 5×pitch}`，相 II 自然把两侧撑开。**这就是 B1 消灭 SpaceBudget 的机制。**
...
| 风险 | 等级 | 缓解 |
|------|------|------|
| 抽象通道图过粗 → 相 II 频繁不可行 | **高** | `RelaxationLadder` L2 允许回相 I 局部重规划；Stage 3 先在小图验证可行率 |
