...
> **执行入口（必读）**：[28-Phase5-启动说明与执行顺序-2026-07.md](28-Phase5-启动说明与执行顺序-2026-07.md)。  
> Phase 4 仅完成**功能里程碑**；架构债 **D4-1～D4-7** 已登记在 [27-Phase4-退出验证](27-Phase4-退出验证-2026-07.md)，**写入本 Phase 范围**，按下表时机执行，不得静默滚到 Phase 6。

#### Phase 4 残余债：何时执行（摘要）

| 时机 | 债 | 说明 |
|---|---|---|
| **Week 0（开工前）** | **D4-1** 写权仪表做实 | 前门禁；未做不开主切片 |
| **Week 1** | **D4-4** 契约单一真源；**D4-2** Group IR 进求解器 | D4-4 是**删 PRS 前置**；D4-2 与 rank 轴并行 |
| **Week 2–3** | **D4-6** 删 PRS / probe / `algo==` + RouteDemand 前馈 | = 下方动作 3–4（主切片） |
| **Week 3–4** | **D4-3** builder 合一；**D4-5** port_solver / EdgeLayout | D4-5 可搭 Phase 6，须在 28 勾选注明 |
| **退出前** | **D4-7** vs `phase4-group-solver` 质量回看 | §8 恢复前处理 |
...
7. **闭合 Phase 4 写权 / IR / builder**（D4-1、D4-2、D4-3）：详见 28；与动作 1 同 Phase 验收，
   不再当作「以后再说」的卫生债。

#### 退出判据

- product-gate 质量轨**优于** Phase 0 基线（这是整个重构的价值兑现点）；
  相对 Phase 4 基线（`phase4-group-solver`）完成 **D4-7** 回看。
- 路由前无任何预览路由；`runner.rs` 中 `algo ==` 字符串比较 = 0。
- 一次 layout run 中坐标求解调用次数 = 1（当前 architecture 最多 3 次）。
- **D4-1 / D4-2 / D4-4 / D4-6 勾选完成**；D4-3 完成或显式开债；D4-5 完成或结转 Phase 6（须笔记）。

---

### Phase 6 — 固化：让补丁无法再生（3–4 天）✅ 2026-07-25
...
| 机制 | 实现 | 状态 |
|---|---|---|
| **写权 typestate** | `FrozenNodeProduct` 字段私有；`GroupShellMut`；`PortAssignment` 只读；materialize→audit→freeze | ✅ |
| **罚项注册表** | `layout/routing/objectives.rs` | ✅ |
| **常量量级检查** | `benchmarks/scripts/check-penalty-ratio.sh` + CI | ✅ |
| **语义隔离检查** | `benchmarks/scripts/check-semantic-isolation.sh` + CI | ✅ |
| **死代码零容忍** | 关键路径脚本（全 crate `deny(dead_code)` 不硬） | ✅ 有界 |
| **写入次数断言** | group 写权阈值 ≤3（目标 =1 记债）；`PLOTGRAM_STRICT_GROUP_WRITES` 可 panic | ✅ 棘轮 |
| **求解器状态可见** | recipe 透传 `orthogonal_debug`；route audit degraded 并入；bench `ortho.degraded_count` | ✅ |

同时：
- ✅ `./benchmarks/snapshot.sh --tag first-principles-final`
- ✅ 手册 §1.0 写入六条公理
- ✅ `AGENTS.md` §8 标注已失效，质量轨恢复

退出验证见 [30-Phase6-退出验证-2026-07.md](30-Phase6-退出验证-2026-07.md)。
...
| 红线 | 检查 | 来源 |
|---|---|---|
| 穿组 = 0 | `edge_crosses_group_interior` | 正确性红线 |
| 确定性 | `det=true` 全样本 | §2 |
| 无图名特判 | CI grep | §5 |
