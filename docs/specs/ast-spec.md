...
本文档定义 Tautcore 的抽象语法树（AST）数据结构。AST 是 Tautcore 的核心数据模型——解析器输出 AST，验证器消费 AST，渲染器消费 AST，Diff/Patch 操作 AST。

**AST 是 Tautcore 的一等公民。** 文本只是 AST 的一种序列化形式。
...
```json
{
    ...
    "constraints": [],
    "style_decls": [],
    "source_info": { "file": "diagram.taut", "line_count": 15 }
}
```
...
本文档中定义的 AST 结构对应 `tautcore-core` crate 中的以下模块：

| AST 概念 | Rust 模块 | 文件 |
|----------|-----------|------|
| 全部结构 | `ast` | `crates/tautcore-core/src/ast.rs` |
...
| Diff / Patch | `diff` | `crates/tautcore-core/src/diff.rs` |
