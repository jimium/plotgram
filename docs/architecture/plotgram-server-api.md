# Plotgram Server API 使用说明

`plotgram-server` 是 Plotgram 的 HTTP 服务，供 Agent、CI、IDE 插件等远程调用解析、校验与渲染能力，无需安装 CLI。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PLOTGRAM_SERVER_ADDR` | `0.0.0.0:6080` | 监听地址 |
| `PLOTGRAM_FONTS_DIR` | （未设置时使用 `cwd/fonts/`） | PNG/WebP 渲染用的 CJK 字体目录 |

```bash
PLOTGRAM_SERVER_ADDR=127.0.0.1:6080 \
PLOTGRAM_FONTS_DIR=/path/to/fonts \
cargo run -p plotgram-server
```
...
对 Plotgram 源码做完整校验：词法/语法解析 + 语义验证。
...
渲染 Plotgram 图表。**渲染前会自动执行与 `/validate` 相同的校验**；只有校验通过才会返回渲染结果。
...
仅接受规范名（与 `GraphicStyleId::as_str()` 一致），**不支持别名**。完整说明见 [`crates/plotgram-core/src/graphic_style/README.md`](../../crates/plotgram-core/src/graphic_style/README.md)。
...
| Header | 说明 | 示例 |
|--------|------|------|
| `X-Plotgram-Format` | 实际输出格式 | `png` |
| `X-Plotgram-Valid` | 校验是否通过 | `true` |
| `X-Plotgram-Warnings` | 警告列表（JSON 数组）；**仅在有警告时出现** | `[{ "code": "W001", ... }]` |
...
- 状态码：`400`
- `Content-Type: application/json`
- Header：`X-Plotgram-Format`、`X-Plotgram-Valid: false`；有警告时附带 `X-Plotgram-Warnings`
...
async function renderPng(source) {
  ...
  const warnings = res.headers.get('X-Plotgram-Warnings');
  if (warnings) {
    console.warn('Plotgram warnings:', JSON.parse(warnings));
  }
...
4. 若 200，直接消费响应 Body（图片/SVG），可选读取 X-Plotgram-Warnings
```
...
| 能力 | plotgram-server | plotgram-wasm | plotgram-cli |
|------|---------------|-------------|------------|
| 远程 HTTP 调用 | ✅ | ❌ | ❌ |
| 浏览器内运行 | ❌ | ✅ | ❌ |
...
```bash
PLOTGRAM_FONTS_DIR=./fonts cargo run -p plotgram-server
```

### 端口被占用？

```bash
PLOTGRAM_SERVER_ADDR=0.0.0.0:6081 cargo run -p plotgram-server
```
...
- `/render` 成功：响应 Header `X-Plotgram-Warnings`（有警告时）
- `/render` 失败：JSON body 的 `check.warnings`，以及 Header `X-Plotgram-Warnings`
