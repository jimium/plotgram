# Cytoscape.js 能力研究：Plotgram 可借鉴的技术与取舍分析
...
它与 Mermaid、Graphviz、Plotgram 处于不同层次：

| 维度 | Cytoscape.js | Plotgram |
|------|-------------|---------|
| 定位 | 通用图可视化 + 交互 + 图分析引擎 | AI 原生图表 DSL + Rust 渲染引擎 |
| 输入 | JSON 元素 + 样式表 | `.pgm` 语义 AST |
...
**核心结论**：Cytoscape.js 是「交互式图分析平台」，Plotgram 是「AI 语义图表编译器」。不应作为运行时依赖引入，但可作为**布局算法与约束求解的参考实现**。

Plotgram 当前代码库中**未引入** Cytoscape.js，这一决策是合理的。
...
以上能力面向**交互式图探索器**，与 Plotgram 的静态导出管线不匹配。
...
**fcose 的关键特性**（对 Plotgram 最有启发）：

> 支持三类用户约束：固定位置（fixed position）、对齐（alignment）、相对放置（relative placement）。

这与 Plotgram 的 [Layout Intent Refinement](./layout-intent-refinement.md) 方向高度吻合：固定位置对应 `Pin`，对齐对应 `Align*`，相对放置对应 `below` / `right_of` 等拓扑意图（经 overlay 表达，非 DSL 坐标 hint）。
...
| Markov 聚类 | `eles.markovClustering()` | 生物网络聚类，与 Plotgram 场景无关 |

---

## 5. 与 Plotgram 现有布局栈的对照

Plotgram 在 `plotgram-core` 中已实现可插拔布局框架（`LayoutStrategy` trait），当前支持的算法：

| Plotgram 布局 | 图表类型默认 | Cytoscape 对标 |
|-------------|-------------|----------------|
...
| `sequence` / `timeline` | sequence | （无直接对标，Plotgram 专属） |

边路由方面，Plotgram 有独立的 `EdgeRoutingStrategy`（orthogonal、spline、bezier 等），Cytoscape.js 的边路由内嵌在 Canvas 渲染中，参考价值较低。
...
| Cytoscape 能力 | Plotgram 对应点 | 行动建议 |
|----------------|----------------|----------|
...
| 能力 | 用途 |
|------|------|
| **compound nodes 模型** | 与 Plotgram `group` 语义对齐：父框由子节点推导，不独立设尺寸 |
...
| 能力 | 原因 |
|------|------|
| 整体作为运行时依赖 | Plotgram 是 Rust 核心 + WASM，引入 JS 图库增加双栈维护成本 |
...
Cytoscape.js 与 Graphviz 在 Plotgram 中的角色类似：**算法参考书，而非运行时依赖**。

| 来源 | 强项 | Plotgram 主要借鉴方向 |
|------|------|---------------------|
...
三者形成互补：Graphviz 提供分层布局的学术最优解，Cytoscape 生态提供约束微调与 compound 图的工程实践，Plotgram 在 Rust 中自行实现并服务于 AI 语义管线。
...
Phase 2（布局意图）
  └─ 研究 fcose/cola 约束模型 → 设计 Plotgram layout intent API
  └─ preset 式坐标保留 + 约束求解器
