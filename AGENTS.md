...
## 8. 【临时】布局路由全局优化期间门禁豁免（2026-07-20 起）

> **状态：生效中**  
> 触发原因：本轮为布局与正交路由的**全局算法级优化**（见 `docs/优化重构/`），目标是让路由和布局达到生产可用水准。优化过程中需要大幅调整管线时序、替换路径搜索内核、重写自环/对称性等模块，必然经历阶段性退化。

**临时规则**：

- **门禁暂停**：本轮优化期间，`benchmarks/compare.sh` 的 product-gate / stress 质量轨**均不阻挡**改动。可以对比基线观察变化，但**无论退步多少都接受**，不作为回滚依据。
- **正确性仍硬**：穿组（`edge_crosses_group_interior`）和确定性（`det=true`）仍然必须满足，这是功能正确性底线。
- **基线可自由重采**：每个阶段完成后直接 `./benchmarks/snapshot.sh --tag <phase>` 重采基线，无需走"抬基线 note"流程。
- **§5 中"无退化要可量化"条款暂停**：不要求节点坐标 diff 持平，不要求 product-gate 严重度不升。
- **§7 创新模式简化**：无需逐次写"目标维度 / 临时退化范围 / 退出判据"三件事；整体优化目标见 `docs/优化重构/02-正交路由与布局优化方案.md`。
